"""
tests/test_institutional_flow_evidence_log.py
=================================================
Self-learning module #30 (ACQUISITION) -- evidence log for real trade
outcomes tagged with an institutional_flow_score.

T01  record_institutional_flow_outcome() persists correctly
...
import learning_system.institutional_flow_evidence_log as evlog
...
def test_t01_record_persists_correctly():
    evlog.record_institutional_flow_outcome(
        order_id="O1", symbol="TEST", strategy="breakout",
        institutional_flow_score=0.5, r_multiple=1.5, won=True,
    )
    records = evlog.get_records()
    assert len(records) == 1
    assert records[0]["institutional_flow_score"] == 0.5
...
def test_t02_get_records_oldest_first_and_n():
    for i in range(5):
        evlog.record_institutional_flow_outcome(
            ...
            institutional_flow_score=0.5, r_multiple=1.0, won=True,
        )
