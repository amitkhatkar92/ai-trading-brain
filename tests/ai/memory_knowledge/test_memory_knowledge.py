...
@pytest.fixture
def gateway():
    from enterprise_ai_platform.ai.memory_knowledge.gateway import MemoryKnowledgeGateway
...
@pytest.fixture
def memory_manager():
    from enterprise_ai_platform.ai.memory_knowledge.memory import MemoryManager
    return MemoryManager()


@pytest.fixture
def knowledge_manager():
    from enterprise_ai_platform.ai.memory_knowledge.knowledge import KnowledgeManager
    return KnowledgeManager()


@pytest.fixture
def retrieval_engine(memory_manager, knowledge_manager):
    from enterprise_ai_platform.ai.memory_knowledge.retrieval import RetrievalEngine
...
@pytest.fixture
def event_bus():
    from enterprise_ai_platform.ai.memory_knowledge.events import MemoryEventBus
    return MemoryEventBus()


@pytest.fixture
def knowledge_graph(event_bus):
    from enterprise_ai_platform.ai.memory_knowledge.graph import KnowledgeGraph
...
class TestMemoryLifecycle:
    ...
    def test_store_returns_entry_with_id(self, memory_manager):
        from enterprise_ai_platform.ai.memory_knowledge.core import MemoryScope
    ...
    def test_retrieve_missing_raises(self, memory_manager):
        from enterprise_ai_platform.ai.memory_knowledge.exceptions import AIMemoryNotFoundError
    ...
    def test_expiry_raises_on_retrieve(self, memory_manager):
        from enterprise_ai_platform.ai.memory_knowledge.core import MemoryScope
        from enterprise_ai_platform.ai.memory_knowledge.exceptions import AIMemoryExpiredError
        past = time.time() - 1
...
class TestKnowledgeLifecycle:
    ...
    def test_add_duplicate_id_raises(self, knowledge_manager):
        from enterprise_ai_platform.ai.memory_knowledge.exceptions import AIKnowledgeAlreadyExistsError
        kid = str(uuid.uuid4())
    ...
    def test_blank_title_raises(self, knowledge_manager):
        from enterprise_ai_platform.ai.memory_knowledge.exceptions import AIKnowledgeValidationError
        with pytest.raises(AIKnowledgeValidationError):
            knowledge_manager.add("   ", "content")

    def test_remove_item(self, knowledge_manager):
        from enterprise_ai_platform.ai.memory_knowledge.exceptions import AIKnowledgeNotFoundError
    ...
    def test_search_by_category(self, knowledge_manager):
        from enterprise_ai_platform.ai.memory_knowledge.core import KnowledgeCategory
...
class TestRetrievalFramework:
    ...
    def test_retrieve_from_knowledge(self, retrieval_engine, knowledge_manager):
        from enterprise_ai_platform.ai.memory_knowledge.retrieval import RetrievalRequest
    ...
    def test_retrieval_with_strategy_swap(self, retrieval_engine, knowledge_manager):
        from enterprise_ai_platform.ai.memory_knowledge.retrieval import RetrievalRequest, HybridRankingStrategy
...
class TestRankingStrategies:
    ...
    def test_keyword_exact_match_score_one(self):
        from enterprise_ai_platform.ai.memory_knowledge.retrieval import KeywordRankingStrategy
    ...
    def test_semantic_stub_returns_half(self):
        from enterprise_ai_platform.ai.memory_knowledge.retrieval import SemanticRankingStrategy
    ...
    def test_hybrid_weights_sum_to_one_required(self):
        from enterprise_ai_platform.ai.memory_knowledge.retrieval import HybridRankingStrategy
    ...
    def test_recency_recent_item_scores_near_one(self):
        from enterprise_ai_platform.ai.memory_knowledge.retrieval import RecencyRankingStrategy
...
class TestPolicyEvaluation:
    ...
    def test_never_expire_retains_all(self):
        from enterprise_ai_platform.ai.memory_knowledge.policy import NeverExpireRetentionPolicy
        from enterprise_ai_platform.ai.memory_knowledge.core import MemoryScope
    ...
    def test_ttl_retention_retains_live(self):
        from enterprise_ai_platform.ai.memory_knowledge.policy import TTLRetentionPolicy
        from enterprise_ai_platform.ai.memory_knowledge.core import MemoryScope
    ...
    def test_scope_retention_correct_scope(self):
        from enterprise_ai_platform.ai.memory_knowledge.policy import ScopeRetentionPolicy
        from enterprise_ai_platform.ai.memory_knowledge.core import MemoryScope
    ...
    def test_unrestricted_retrieval_allows_all(self):
        from enterprise_ai_platform.ai.memory_knowledge.policy import UnrestrictedRetrievalPolicy
        from enterprise_ai_platform.ai.memory_knowledge.core import MemoryScope
    ...
    def test_owner_only_retrieval_matches(self):
        from enterprise_ai_platform.ai.memory_knowledge.policy import OwnerOnlyRetrievalPolicy
        from enterprise_ai_platform.ai.memory_knowledge.core import MemoryScope
    ...
    def test_permissive_privacy_allows_all(self):
        from enterprise_ai_platform.ai.memory_knowledge.policy import PermissivePrivacyPolicy
        from enterprise_ai_platform.ai.memory_knowledge.core import MemoryScope
    ...
    def test_scope_restricted_privacy_working(self):
        from enterprise_ai_platform.ai.memory_knowledge.policy import ScopeRestrictedPrivacyPolicy
        from enterprise_ai_platform.ai.memory_knowledge.core import MemoryScope
    ...
    def test_no_expiration_returns_none(self):
        from enterprise_ai_platform.ai.memory_knowledge.policy import NoExpirationPolicy
        from enterprise_ai_platform.ai.memory_knowledge.core import MemoryScope
        policy = NoExpirationPolicy()
        assert policy.expires_at(MemoryScope.SESSION) is None

    def test_ttl_expiration_working_scope(self):
        from enterprise_ai_platform.ai.memory_knowledge.policy import TTLExpirationPolicy
        from enterprise_ai_platform.ai.memory_knowledge.core import MemoryScope
    ...
    def test_default_ranking_policy_returns_keyword(self):
        from enterprise_ai_platform.ai.memory_knowledge.policy import DefaultRankingPolicy
        policy = DefaultRankingPolicy()
        assert policy.preferred_strategy() == "keyword"

    def test_semantic_ranking_policy(self):
        from enterprise_ai_platform.ai.memory_knowledge.policy import SemanticRankingPolicy
        assert SemanticRankingPolicy().preferred_strategy() == "semantic"

    def test_hybrid_ranking_policy(self):
        from enterprise_ai_platform.ai.memory_knowledge.policy import HybridRankingPolicy
...
class TestEventGeneration:
    ...
    def test_memory_created_event_published(self, event_bus):
        from enterprise_ai_platform.ai.memory_knowledge.events import MemoryEventType
        from enterprise_ai_platform.ai.memory_knowledge.memory import MemoryManager
        from enterprise_ai_platform.ai.memory_knowledge.core   import MemoryScope
        received = []
    ...
    def test_retrieval_completed_event_published(self, event_bus):
        from enterprise_ai_platform.ai.memory_knowledge.events     import MemoryEventType
        from enterprise_ai_platform.ai.memory_knowledge.retrieval  import RetrievalEngine, RetrievalRequest
        received = []
    ...
    def test_graph_traversed_event_published(self, event_bus):
        from enterprise_ai_platform.ai.memory_knowledge.events import MemoryEventType
        from enterprise_ai_platform.ai.memory_knowledge.graph  import KnowledgeGraph, KnowledgeNode
        received = []
    ...
    def test_event_bus_published_count_increments(self, event_bus):
        from enterprise_ai_platform.ai.memory_knowledge.events.memory_events import MemoryCreatedEvent
    ...
    def test_event_bus_unsubscribe(self, event_bus):
        ...
        event_bus.publish(
            __import__(
                "enterprise_ai_platform.ai.memory_knowledge.events.memory_events",
...
class TestKnowledgeGraph:
    ...
    def test_add_and_get_node(self, knowledge_graph):
        from enterprise_ai_platform.ai.memory_knowledge.graph import KnowledgeNode
    ...
    def test_add_relationship(self, knowledge_graph):
        from enterprise_ai_platform.ai.memory_knowledge.graph import KnowledgeNode, KnowledgeRelationship
...
class TestVectorAbstractions:
    ...
    def test_vector_store_is_abstract(self):
        from enterprise_ai_platform.ai.memory_knowledge.vector import VectorStore
        with pytest.raises(TypeError):
            VectorStore()  # type: ignore

    def test_embedding_service_is_abstract(self):
        from enterprise_ai_platform.ai.memory_knowledge.vector import EmbeddingService
        with pytest.raises(TypeError):
            EmbeddingService()  # type: ignore

    def test_similarity_search_is_abstract(self):
        from enterprise_ai_platform.ai.memory_knowledge.vector import SimilaritySearch
        with pytest.raises(TypeError):
            SimilaritySearch()  # type: ignore

    def test_vector_index_is_abstract(self):
        from enterprise_ai_platform.ai.memory_knowledge.vector import VectorIndex
        with pytest.raises(TypeError):
    ...
    def test_concrete_store_implements_all_methods(self):
        ...
        from enterprise_ai_platform.ai.memory_knowledge.memory import InMemoryStore
        from enterprise_ai_platform.ai.memory_knowledge.core   import MemoryScope, MemoryEntry
        store = InMemoryStore()
...
class TestSnapshot:
    def test_snapshot_captures_counts(self, gateway):
        from enterprise_ai_platform.ai.memory_knowledge.core import MemoryScope, KnowledgeCategory
...
class TestContainerDIWiring:
    def test_build_is_idempotent(self):
        from enterprise_ai_platform.ai.memory_knowledge.container import MemoryKnowledgeContainer
...
class TestExceptionHierarchy:
    ...
    def test_error_codes(self, exc_cls, expected_code):
        import enterprise_ai_platform.ai.memory_knowledge.exceptions as exc_mod
