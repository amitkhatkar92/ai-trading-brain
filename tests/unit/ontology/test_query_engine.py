...
def _reset_all() -> None:
    ...
    from enterprise_ai_platform.ontology.query.query_engine        import reset_query_engine_v2
    from enterprise_ai_platform.ontology.query.query_manager       import reset_query_manager
    from enterprise_ai_platform.ontology.query.resolution_engine   import reset_resolution_engine
    from enterprise_ai_platform.ontology.query.semantic_engine     import reset_semantic_engine
    from enterprise_ai_platform.ontology.query.query_registry      import reset_query_registry
    from enterprise_ai_platform.ontology.query.query_cache         import reset_query_cache
    from enterprise_ai_platform.ontology.query.query_optimizer     import reset_query_optimizer
    from enterprise_ai_platform.ontology.query.query_factory       import reset_query_factory
    from enterprise_ai_platform.ontology.query.query_context       import reset_query_context
...
def _make_type(
    name:   str,
    ns:     str = "enterprise_ai_platform.test",
...
def _register_rel(
    ...
    rel = OntologyRelationshipDef(
        uri             = uri,
        name            = name,
        namespace_uri   = "enterprise_ai_platform.test",
...
def _build_hierarchy():
    ...
    """
    types = {
        "asset":  _make_type("Asset",  ns="enterprise_ai_platform.test", abstract=True),
        "equity": _make_type("Equity", ns="enterprise_ai_platform.test", parent="enterprise_ai_platform.test.Asset"),
        "stock":  _make_type("Stock",  ns="enterprise_ai_platform.test", parent="enterprise_ai_platform.test.Equity"),
        "bond":   _make_type("Bond",   ns="enterprise_ai_platform.test", parent="enterprise_ai_platform.test.Asset"),
    }
...
class TestQueryConstants:
    def test_query_type_values(self):
        from enterprise_ai_platform.ontology.query import QueryType
    ...
    def test_resolution_strategy_values(self):
        from enterprise_ai_platform.ontology.query import ResolutionStrategy
    ...
    def test_navigation_direction_values(self):
        from enterprise_ai_platform.ontology.query import NavigationDirection
        assert NavigationDirection.UP.value   == "up"
        assert NavigationDirection.DOWN.value == "down"

    def test_semantic_relation_values(self):
        from enterprise_ai_platform.ontology.query import SemanticRelation
...
class TestQueryContext:
    def test_singleton(self):
        from enterprise_ai_platform.ontology.query import get_query_context
        ctx1 = get_query_context()
        ctx2 = get_query_context()
        assert ctx1 is ctx2

    def test_reset(self):
        from enterprise_ai_platform.ontology.query import get_query_context, reset_query_context
        ctx1 = get_query_context()
        reset_query_context()
        ctx2 = get_query_context()
    ...
    def test_query_operation_context(self):
        from enterprise_ai_platform.ontology.query import get_query_context, QueryType
    ...
    def test_resolution_context_increments_depth(self):
        from enterprise_ai_platform.ontology.query import get_query_context, ResolutionStrategy
        ctx = get_query_context()
        assert ctx.depth == 0
    ...
    def test_diagnostics(self):
        from enterprise_ai_platform.ontology.query import get_query_context, QueryDiagnosticLevel, QueryType
        ctx = get_query_context()
...
class TestQueryFactory:
    def test_singleton(self):
        from enterprise_ai_platform.ontology.query import get_query_factory
        assert get_query_factory() is get_query_factory()

    def test_make_query(self):
        from enterprise_ai_platform.ontology.query import get_query_factory, QueryType, SortOrder
    ...
    def test_make_resolution(self):
        from enterprise_ai_platform.ontology.query import get_query_factory, ResolutionStrategy
        fac = get_query_factory()
    ...
    def test_make_navigation(self):
        from enterprise_ai_platform.ontology.query import get_query_factory, NavigationDirection
        fac = get_query_factory()
    ...
    def test_make_semantic(self):
        from enterprise_ai_platform.ontology.query import get_query_factory
        fac = get_query_factory()
        req = fac.make_semantic("enterprise_ai_platform.test.Asset", top_k=5, radius=2)
        assert req.type_uri == "enterprise_ai_platform.test.Asset"
        assert req.top_k    == 5

    def test_make_query_result_to_dict(self):
        from enterprise_ai_platform.ontology.query import get_query_factory, QueryType, QueryStatus
        fac = get_query_factory()
    ...
    def test_similar_type_to_dict(self):
        from enterprise_ai_platform.ontology.query import SimilarType, SemanticRelation
        td   = _make_type("X")
...
class TestQueryCache:
    def test_singleton(self):
        from enterprise_ai_platform.ontology.query import get_query_cache
        assert get_query_cache() is get_query_cache()

    def test_put_get(self):
        from enterprise_ai_platform.ontology.query import get_query_cache
        cache = get_query_cache()
        key   = cache.make_key("type_lookup", "enterprise_ai_platform.test.X")
    ...
    def test_lru_eviction(self):
        from enterprise_ai_platform.ontology.query import get_query_cache, reset_query_cache
        reset_query_cache()
        from enterprise_ai_platform.ontology.query.query_cache import QueryCache
    ...
    def test_make_key_deterministic(self):
        from enterprise_ai_platform.ontology.query import get_query_cache
        cache = get_query_cache()
        k1 = cache.make_key("type_lookup", "enterprise_ai_platform.test.A")
        k2 = cache.make_key("type_lookup", "enterprise_ai_platform.test.A")
        assert k1 == k2
...
class TestQueryOptimizer:
    def test_singleton(self):
        from enterprise_ai_platform.ontology.query import get_query_optimizer
        assert get_query_optimizer() is get_query_optimizer()

    def test_plan_for_type_lookup(self):
        from enterprise_ai_platform.ontology.query import get_query_optimizer, get_query_factory, QueryType, IndexHint
        factory   = get_query_factory()
        optimizer = get_query_optimizer()
        req  = factory.make_query(QueryType.TYPE_LOOKUP, "enterprise_ai_platform.test.Asset")
    ...
    def test_plan_for_hierarchy(self):
        ...
        factory   = get_query_factory()
        optimizer = get_query_optimizer()
        req  = factory.make_query(QueryType.HIERARCHY, "enterprise_ai_platform.test.Asset")
        plan = optimizer.plan(req)
        assert plan.index_hint == IndexHint.HIERARCHY_INDEX

    def test_plan_namespace_hint(self):
        from enterprise_ai_platform.ontology.query import get_query_optimizer, get_query_factory, QueryType
        factory   = get_query_factory()
        optimizer = get_query_optimizer()
        req  = factory.make_query(QueryType.TYPE_LOOKUP, "Asset", namespace_hint="enterprise_ai_platform.entity")
        plan = optimizer.plan(req)
...
class TestQueryRegistry:
    def test_singleton(self):
        from enterprise_ai_platform.ontology.query import get_query_registry
        assert get_query_registry() is get_query_registry()

    def test_builtin_queries_registered(self):
        from enterprise_ai_platform.ontology.query import get_query_registry, QID_ALL_TYPES, QID_ALL_RELATIONSHIPS
    ...
    def test_builtin_queries_are_flagged(self):
        from enterprise_ai_platform.ontology.query import get_query_registry, QID_ALL_TYPES
        reg = get_query_registry()
        nq  = reg.get(QID_ALL_TYPES)
        assert nq.builtin is True

    def test_register_custom(self):
        from enterprise_ai_platform.ontology.query import get_query_registry, QueryType
        reg = get_query_registry()
        reg.register(
    ...
    def test_duplicate_raises(self):
        from enterprise_ai_platform.ontology.query import get_query_registry, QueryType, DuplicateNamedQueryError
        reg = get_query_registry()
    ...
    def test_cannot_unregister_builtin(self):
        from enterprise_ai_platform.ontology.query import get_query_registry, QID_ALL_TYPES, NamedQueryError
        reg = get_query_registry()
        with pytest.raises(NamedQueryError):
            reg.unregister(QID_ALL_TYPES)

    def test_get_unknown_raises(self):
        from enterprise_ai_platform.ontology.query import get_query_registry, UnknownNamedQueryError
...
class TestResolutionEngine:
    def test_singleton(self):
        from enterprise_ai_platform.ontology.query import get_resolution_engine
        assert get_resolution_engine() is get_resolution_engine()

    def test_exact_resolution(self):
        from enterprise_ai_platform.ontology.query import get_resolution_engine, ResolutionStrategy
        types = _build_hierarchy()
        res   = get_resolution_engine()
        td    = res.resolve("enterprise_ai_platform.test.Asset", ResolutionStrategy.EXACT)
    ...
    def test_auto_strategy(self):
        ...
        types = _build_hierarchy()
        res   = get_resolution_engine()
        td    = res.resolve("enterprise_ai_platform.test.Bond", ResolutionStrategy.AUTO)
        assert td is not None
        assert td.name == "Bond"

    def test_resolve_or_raise_raises(self):
        from enterprise_ai_platform.ontology.query import get_resolution_engine, ResolutionStrategy, ResolutionError
        res = get_resolution_engine()
    ...
    def test_resolve_canonical(self):
        ...
        types = _build_hierarchy()
        res   = get_resolution_engine()
        canon = res.resolve_canonical("enterprise_ai_platform.test.Asset")
        assert canon == "enterprise_ai_platform.test.Asset"

    def test_inheritance_chain(self):
        from enterprise_ai_platform.ontology.query import get_resolution_engine
        _build_hierarchy()
        res   = get_resolution_engine()
        chain = res.resolve_inheritance_chain("enterprise_ai_platform.test.Stock")
        uris  = [t.uri for t in chain]
        assert uris[0] == "enterprise_ai_platform.test.Stock"
        assert "enterprise_ai_platform.test.Equity" in uris
        assert "enterprise_ai_platform.test.Asset"  in uris

    def test_inheritance_chain_single_root(self):
        from enterprise_ai_platform.ontology.query import get_resolution_engine
        _build_hierarchy()
        res   = get_resolution_engine()
        chain = res.resolve_inheritance_chain("enterprise_ai_platform.test.Asset")
        assert len(chain) == 1
        assert chain[0].uri == "enterprise_ai_platform.test.Asset"

    def test_resolve_all_properties(self):
        from enterprise_ai_platform.ontology.query import get_resolution_engine
        from enterprise_ai_platform.ontology.runtime.runtime_object import OntologyProperty, DataType
        _build_hierarchy()
        ...
        with mgr._lock:
            mgr._types["enterprise_ai_platform.test.Asset"].properties["isin"] = prop

        res   = get_resolution_engine()
        props = res.resolve_all_properties("enterprise_ai_platform.test.Stock")
        # Stock inherits from Equity → Asset, so isin should be present
        assert "isin" in props

    def test_resolve_relationship_by_uri(self):
        from enterprise_ai_platform.ontology.query import get_resolution_engine
        _register_rel("enterprise_ai_platform.test.rel.HasEquity", "HasEquity", "enterprise_ai_platform.test.Asset", "enterprise_ai_platform.test.Equity")
        res = get_resolution_engine()
        rel = res.resolve_relationship("enterprise_ai_platform.test.rel.HasEquity")
        assert rel is not None
        assert rel.name == "HasEquity"

    def test_resolve_relationship_by_name(self):
        from enterprise_ai_platform.ontology.query import get_resolution_engine
        _register_rel("enterprise_ai_platform.test.rel.HasBond", "HasBond", "enterprise_ai_platform.test.Asset", "enterprise_ai_platform.test.Bond")
        res = get_resolution_engine()
    ...
    def test_request_api(self):
        from enterprise_ai_platform.ontology.query import get_resolution_engine, get_query_factory, ResolutionStrategy
        _build_hierarchy()
        factory = get_query_factory()
        req     = factory.make_resolution("enterprise_ai_platform.test.Bond", ResolutionStrategy.EXACT)
        res     = get_resolution_engine()
    ...
    def test_stats_track_counts(self):
        ...
        _build_hierarchy()
        res = get_resolution_engine()
        res.resolve("enterprise_ai_platform.test.Asset", ResolutionStrategy.AUTO)
        res.resolve_inheritance_chain("enterprise_ai_platform.test.Stock")
...
class TestSemanticEngine:
    def test_singleton(self):
        from enterprise_ai_platform.ontology.query import get_semantic_engine
    ...
    def test_find_similar_returns_list(self):
        ...
        sims = sem.find_similar("enterprise_ai_platform.test.Asset", top_k=10)
    ...
    def test_find_similar_excludes_self(self):
        ...
        assert "enterprise_ai_platform.test.Asset" not in uris

    def test_semantic_distance_same_type(self):
        from enterprise_ai_platform.ontology.query import get_semantic_engine
        _build_hierarchy()
        sem = get_semantic_engine()
        assert sem.semantic_distance("enterprise_ai_platform.test.Asset", "enterprise_ai_platform.test.Asset") == 0.0

    def test_semantic_distance_parent_child(self):
        from enterprise_ai_platform.ontology.query import get_semantic_engine, SEMANTIC_DISTANCE_INFINITY
        _build_hierarchy()
        sem  = get_semantic_engine()
        dist = sem.semantic_distance("enterprise_ai_platform.test.Equity", "enterprise_ai_platform.test.Asset")
        assert 0.0 < dist < SEMANTIC_DISTANCE_INFINITY

    def test_semantic_distance_unrelated(self):
        from enterprise_ai_platform.ontology.query import get_semantic_engine, SEMANTIC_DISTANCE_INFINITY
        # Two types with no common ancestor
        td_x = _make_type("IsolatedX", ns="enterprise_ai_platform.isolated")
        td_y = _make_type("IsolatedY", ns="enterprise_ai_platform.isolated")
        _register_type(td_x)
        _register_type(td_y)
        sem  = get_semantic_engine()
        dist = sem.semantic_distance("enterprise_ai_platform.isolated.IsolatedX", "enterprise_ai_platform.isolated.IsolatedY")
    ...
    def test_is_semantically_equivalent_same_uri(self):
        ...
        _build_hierarchy()
        sem = get_semantic_engine()
        assert sem.is_semantically_equivalent("enterprise_ai_platform.test.Asset", "enterprise_ai_platform.test.Asset") is True
    ...
    def test_is_semantically_equivalent_alias(self):
        ...
        assert sem.is_semantically_equivalent("enterprise_ai_platform.test.GovBond", "enterprise_ai_platform.test.GovBond") is True

    def test_expand_concept(self):
        from enterprise_ai_platform.ontology.query import get_semantic_engine
        _build_hierarchy()
        sem       = get_semantic_engine()
        expanded  = sem.expand_concept("enterprise_ai_platform.test.Asset", radius=3)
        uris      = [t.uri for t in expanded]
        assert "enterprise_ai_platform.test.Equity" in uris
        assert "enterprise_ai_platform.test.Bond"   in uris
    ...
    def test_expand_concept_radius_zero(self):
        ...
        expanded = sem.expand_concept("enterprise_ai_platform.test.Asset", radius=0)
        assert expanded == []
    ...
    def test_discover_neighborhood(self):
        ...
        _build_hierarchy()
        sem  = get_semantic_engine()
        hood = sem.discover_neighborhood("enterprise_ai_platform.test.Equity", depth=2)
        assert "parents"  in hood
        assert "children" in hood
        assert "enterprise_ai_platform.test.Asset" in hood["parents"]
        assert "enterprise_ai_platform.test.Stock" in hood["children"]

    def test_find_related_subtype(self):
        from enterprise_ai_platform.ontology.query import get_semantic_engine, SemanticRelation
        _build_hierarchy()
        sem     = get_semantic_engine()
        related = sem.find_related("enterprise_ai_platform.test.Asset", SemanticRelation.SUBTYPE_OF)
    ...
    def test_find_related_supertype(self):
        ...
        _build_hierarchy()
        sem     = get_semantic_engine()
        related = sem.find_related("enterprise_ai_platform.test.Stock", SemanticRelation.SUPERTYPE_OF)
        uris    = [t.uri for t in related]
        assert "enterprise_ai_platform.test.Equity" in uris
        assert "enterprise_ai_platform.test.Asset"  in uris
    ...
    def test_find_related_sibling(self):
        ...
        _build_hierarchy()
        sem      = get_semantic_engine()
        siblings = sem.find_related("enterprise_ai_platform.test.Equity", SemanticRelation.SIBLING)
    ...
    def test_stats_tracking(self):
        ...
        _build_hierarchy()
        sem = get_semantic_engine()
        sem.find_similar("enterprise_ai_platform.test.Asset")
        sem.expand_concept("enterprise_ai_platform.test.Asset")
        sem.semantic_rank([], "x")
        s = sem.stats()
    ...
    def test_request_api(self):
        from enterprise_ai_platform.ontology.query import get_semantic_engine, get_query_factory
        _build_hierarchy()
        factory = get_query_factory()
        req     = factory.make_semantic("enterprise_ai_platform.test.Asset", top_k=5, radius=2)
        sem     = get_semantic_engine()
...
class TestQueryManager:
    def test_singleton(self):
        from enterprise_ai_platform.ontology.query import get_query_manager
    ...
    def test_lookup_type(self):
        ...
        _build_hierarchy()
        mgr = get_query_manager()
        td  = mgr.lookup_type("enterprise_ai_platform.test.Asset")
    ...
    def test_lookup_type_or_raise(self):
        from enterprise_ai_platform.ontology.query import get_query_manager, QueryNotFoundError
        mgr = get_query_manager()
    ...
    def test_exists(self):
        ...
        _build_hierarchy()
        mgr = get_query_manager()
        assert mgr.exists("enterprise_ai_platform.test.Asset") is True
    ...
    def test_parent_of(self):
        ...
        _build_hierarchy()
        mgr = get_query_manager()
        parent = mgr.parent_of("enterprise_ai_platform.test.Stock")
        assert parent.uri == "enterprise_ai_platform.test.Equity"

    def test_parent_of_root_returns_none(self):
        from enterprise_ai_platform.ontology.query import get_query_manager
        _build_hierarchy()
        mgr = get_query_manager()
        assert mgr.parent_of("enterprise_ai_platform.test.Asset") is None

    def test_children_of(self):
        from enterprise_ai_platform.ontology.query import get_query_manager
        _build_hierarchy()
        mgr      = get_query_manager()
        children = mgr.children_of("enterprise_ai_platform.test.Asset")
    ...
    def test_ancestors_of(self):
        ...
        _build_hierarchy()
        mgr       = get_query_manager()
        ancestors = mgr.ancestors_of("enterprise_ai_platform.test.Stock")
    ...
    def test_descendants_of(self):
        ...
        _build_hierarchy()
        mgr         = get_query_manager()
        descendants = mgr.descendants_of("enterprise_ai_platform.test.Asset")
        uris        = [t.uri for t in descendants]
        assert "enterprise_ai_platform.test.Equity" in uris
        assert "enterprise_ai_platform.test.Stock"  in uris
        assert "enterprise_ai_platform.test.Bond"   in uris
    ...
    def test_is_subtype_of(self):
        ...
        _build_hierarchy()
        mgr = get_query_manager()
        assert mgr.is_subtype_of("enterprise_ai_platform.test.Stock",  "enterprise_ai_platform.test.Asset")  is True
        assert mgr.is_subtype_of("enterprise_ai_platform.test.Asset",  "enterprise_ai_platform.test.Stock")  is False
        assert mgr.is_subtype_of("enterprise_ai_platform.test.Asset",  "enterprise_ai_platform.test.Asset")  is True
    ...
    def test_properties_of_inherited(self):
        ...
        with mgr2._lock:
            mgr2._types["enterprise_ai_platform.test.Asset"].properties["isin"] = prop

        mgr   = get_query_manager()
        props = mgr.properties_of("enterprise_ai_platform.test.Stock", inherited=True)
        assert "isin" in props

    def test_lookup_relationship(self):
        from enterprise_ai_platform.ontology.query import get_query_manager
        _register_rel("enterprise_ai_platform.test.rel.Owns", "Owns", "enterprise_ai_platform.test.Asset", "enterprise_ai_platform.test.Equity")
        mgr = get_query_manager()
        rel = mgr.lookup_relationship("enterprise_ai_platform.test.rel.Owns")
        assert rel is not None and rel.name == "Owns"

    def test_relationships_for(self):
        from enterprise_ai_platform.ontology.query import get_query_manager
        _build_hierarchy()
        _register_rel("enterprise_ai_platform.test.rel.Contains", "Contains", "enterprise_ai_platform.test.Asset", "enterprise_ai_platform.test.Bond")
        mgr  = get_query_manager()
        rels = mgr.relationships_for("enterprise_ai_platform.test.Asset")
    ...
    def test_search_with_namespace_hint(self):
        from enterprise_ai_platform.ontology.query import get_query_manager
        td = _make_type("EquityAlpha", ns="enterprise_ai_platform.special")
        _register_type(td)
        mgr     = get_query_manager()
        results = mgr.search("EquityAlpha", namespace_hint="enterprise_ai_platform.special")
        assert all(t.namespace_uri == "enterprise_ai_platform.special" for t in results)
    ...
    def test_execute_query_type_lookup(self):
        from enterprise_ai_platform.ontology.query import get_query_manager, get_query_factory, QueryType, QueryStatus
        _build_hierarchy()
        factory = get_query_factory()
        req     = factory.make_query(QueryType.TYPE_LOOKUP, "enterprise_ai_platform.test.Asset")
        mgr     = get_query_manager()
    ...
    def test_execute_query_cached_second_call(self):
        ...
        _build_hierarchy()
        factory = get_query_factory()
        req     = factory.make_query(QueryType.TYPE_LOOKUP, "enterprise_ai_platform.test.Equity")
        mgr     = get_query_manager()
        mgr.execute_query(req)
        # Second call with same params should hit cache
        # Make a new request with same params to trigger cache check
        req2    = factory.make_query(QueryType.TYPE_LOOKUP, "enterprise_ai_platform.test.Equity")
    ...
    def test_execute_query_ancestors(self):
        from enterprise_ai_platform.ontology.query import get_query_manager, get_query_factory, QueryType
        _build_hierarchy()
        factory = get_query_factory()
        req     = factory.make_query(QueryType.ANCESTORS, "enterprise_ai_platform.test.Stock")
        mgr     = get_query_manager()
        result  = mgr.execute_query(req)
    ...
    def test_invalidate_cache(self):
        from enterprise_ai_platform.ontology.query import get_query_manager, get_query_cache
        mgr = get_query_manager()
        cache = get_query_cache()
...
class TestQueryEngine:
    def test_singleton(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2
    ...
    def test_type_lookup(self):
        ...
        _build_hierarchy()
        eng = get_query_engine_v2()
        td  = eng.type("enterprise_ai_platform.test.Asset")
        assert td is not None and td.name == "Asset"
    ...
    def test_type_or_raise(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2, QueryNotFoundError
        eng = get_query_engine_v2()
        with pytest.raises(QueryNotFoundError):
    ...
    def test_has_type(self):
        ...
        _build_hierarchy()
        eng = get_query_engine_v2()
        assert eng.has_type("enterprise_ai_platform.test.Asset") is True
    ...
    def test_canonical_uri(self):
        ...
        _build_hierarchy()
        eng   = get_query_engine_v2()
        canon = eng.canonical_uri("enterprise_ai_platform.test.Bond")
        assert canon == "enterprise_ai_platform.test.Bond"

    def test_parent_of(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2
        _build_hierarchy()
        eng = get_query_engine_v2()
        p   = eng.parent_of("enterprise_ai_platform.test.Equity")
        assert p.uri == "enterprise_ai_platform.test.Asset"

    def test_children_of(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2
        _build_hierarchy()
        eng      = get_query_engine_v2()
        children = eng.children_of("enterprise_ai_platform.test.Asset")
        uris     = [t.uri for t in children]
        assert "enterprise_ai_platform.test.Equity" in uris
    ...
    def test_ancestors_of(self):
        ...
        _build_hierarchy()
        eng       = get_query_engine_v2()
        ancestors = eng.ancestors_of("enterprise_ai_platform.test.Stock")
        uris      = [t.uri for t in ancestors]
        assert "enterprise_ai_platform.test.Asset" in uris
    ...
    def test_descendants_of(self):
        ...
        _build_hierarchy()
        eng         = get_query_engine_v2()
        descendants = eng.descendants_of("enterprise_ai_platform.test.Asset")
        uris        = [t.uri for t in descendants]
        assert "enterprise_ai_platform.test.Stock" in uris
    ...
    def test_inheritance_chain(self):
        ...
        _build_hierarchy()
        eng   = get_query_engine_v2()
        chain = eng.inheritance_chain("enterprise_ai_platform.test.Stock")
        uris  = [t.uri for t in chain]
        assert uris[0] == "enterprise_ai_platform.test.Stock"
    ...
    def test_is_subtype_of(self):
        ...
        _build_hierarchy()
        eng = get_query_engine_v2()
        assert eng.is_subtype_of("enterprise_ai_platform.test.Bond", "enterprise_ai_platform.test.Asset") is True

    def test_relationship(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2
        _register_rel("enterprise_ai_platform.test.r.Covers", "Covers", "enterprise_ai_platform.test.Asset", "enterprise_ai_platform.test.Bond")
        eng = get_query_engine_v2()
        rel = eng.relationship("enterprise_ai_platform.test.r.Covers")
        assert rel is not None

    def test_all_relationships(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2
        _register_rel("enterprise_ai_platform.test.r.EngRel", "EngRel", "enterprise_ai_platform.test.Asset", "enterprise_ai_platform.test.Bond")
        eng  = get_query_engine_v2()
    ...
    def test_similar_types(self):
        ...
        _build_hierarchy()
        eng  = get_query_engine_v2()
        sims = eng.similar_types("enterprise_ai_platform.test.Asset", top_k=5)
        assert isinstance(sims, list)
    ...
    def test_semantic_distance(self):
        ...
        _build_hierarchy()
        eng  = get_query_engine_v2()
        dist = eng.semantic_distance("enterprise_ai_platform.test.Equity", "enterprise_ai_platform.test.Bond")
        assert dist >= 0.0
    ...
    def test_expand_concept(self):
        ...
        _build_hierarchy()
        eng      = get_query_engine_v2()
        expanded = eng.expand_concept("enterprise_ai_platform.test.Asset", radius=2)
        uris     = [t.uri for t in expanded]
        assert "enterprise_ai_platform.test.Equity" in uris
    ...
    def test_neighborhood(self):
        ...
        _build_hierarchy()
        eng  = get_query_engine_v2()
        hood = eng.neighborhood("enterprise_ai_platform.test.Equity")
        assert "parents"  in hood
        assert "children" in hood

    def test_find_related_subtype(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2, SemanticRelation
        _build_hierarchy()
        eng     = get_query_engine_v2()
        related = eng.find_related("enterprise_ai_platform.test.Asset", SemanticRelation.SUBTYPE_OF)
        uris    = [t.uri for t in related]
        assert "enterprise_ai_platform.test.Bond" in uris
    ...
    def test_resolve_alias(self):
        ...
        assert canon == "enterprise_ai_platform.test.IndexFund"
    ...
    def test_invalidate_cache(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2, get_query_cache
        eng   = get_query_engine_v2()
        cache = get_query_cache()
        cache.put("k", "v")
    ...
    def test_register_named_query(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2, get_query_registry, QueryType
        eng = get_query_engine_v2()
    ...
    def test_execute_named(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2, QueryType
        _build_hierarchy()
        eng = get_query_engine_v2()
        eng.register_named_query(
            ...
            default_target = "enterprise_ai_platform.test.Asset",
        )
    ...
    def test_is_semantically_equivalent(self):
        ...
        _build_hierarchy()
        eng = get_query_engine_v2()
        assert eng.is_semantically_equivalent("enterprise_ai_platform.test.Asset", "enterprise_ai_platform.test.Asset") is True
        assert eng.is_semantically_equivalent("enterprise_ai_platform.test.Asset", "enterprise_ai_platform.test.Bond")  is False
...
class TestFluentAndNavigation:
    def test_fluent_basic(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2, QueryType, QueryStatus
        _build_hierarchy()
        eng    = get_query_engine_v2()
        result = eng.query(QueryType.TYPE_LOOKUP, "enterprise_ai_platform.test.Bond").execute()
        assert result.status == QueryStatus.COMPLETED
        assert result.count  == 1
    ...
    def test_fluent_limit(self):
        ...
        _build_hierarchy()
        eng    = get_query_engine_v2()
        result = eng.query(QueryType.ANCESTORS, "enterprise_ai_platform.test.Stock").limit(1).execute()
        assert result.count <= 1

    def test_fluent_namespace(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2, QueryType
        td = _make_type("NsFilterTest", ns="enterprise_ai_platform.filter")
        _register_type(td)
        eng    = get_query_engine_v2()
        result = (eng.query(QueryType.SEARCH, "NsFilterTest")
                     .namespace("enterprise_ai_platform.filter")
                     .execute())
        uris = result.uris()
        assert "enterprise_ai_platform.filter.NsFilterTest" in uris

    def test_navigation_down(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2
        _build_hierarchy()
        eng    = get_query_engine_v2()
        result = eng.navigation_request("enterprise_ai_platform.test.Asset", max_depth=4)
        uris   = [t.uri for t in result.visited]
        assert "enterprise_ai_platform.test.Equity" in uris

    def test_navigation_up(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2, NavigationDirection
        _build_hierarchy()
        eng    = get_query_engine_v2()
        result = eng.navigation_request(
            "enterprise_ai_platform.test.Stock",
    ...
    def test_navigation_stop_at(self):
        from enterprise_ai_platform.ontology.query import get_query_manager, get_query_factory, NavigationDirection
        _build_hierarchy()
        factory = get_query_factory()
        req     = factory.make_navigation(
            "enterprise_ai_platform.test.Asset",
            direction = NavigationDirection.DOWN,
            max_depth = 8,
            stop_at   = "enterprise_ai_platform.test.Equity",
        )
        mgr    = get_query_manager()
        result = mgr.execute_navigation(req)
        assert "enterprise_ai_platform.test.Equity" in result.path

    def test_resolution_request_api(self):
        from enterprise_ai_platform.ontology.query import get_query_engine_v2, ResolutionStrategy
        _build_hierarchy()
        eng    = get_query_engine_v2()
        result = eng.resolution_request("enterprise_ai_platform.test.Bond", ResolutionStrategy.EXACT)
        assert result.succeeded
        assert result.resolved.name == "Bond"
    ...
    def test_semantic_request_api(self):
        ...
        _build_hierarchy()
        eng    = get_query_engine_v2()
        result = eng.semantic_request("enterprise_ai_platform.test.Asset", top_k=3, radius=2)
        assert isinstance(result.similar, list)
...
class TestConcurrency:
    def test_parallel_type_lookups(self):
        ...
        uris    = ["enterprise_ai_platform.test.Asset", "enterprise_ai_platform.test.Equity", "enterprise_ai_platform.test.Stock", "enterprise_ai_platform.test.Bond"]
        ...
        assert errors == []
        assert results["enterprise_ai_platform.test.Asset"]  == "Asset"
        assert results["enterprise_ai_platform.test.Equity"] == "Equity"
...
class TestLargeOntology:
    def _build_large(self, n: int = 100) -> list:
        ...
        root = _make_type("Root", ns="enterprise_ai_platform.large")
        _register_type(root)
        types = [root]
        for i in range(n):
            td = _make_type(f"Type{i:04d}", ns="enterprise_ai_platform.large", parent="enterprise_ai_platform.large.Root")
            _register_type(td)
            types.append(td)
        return types
    ...
    def test_large_search(self):
        ...
        eng     = get_query_engine_v2()
        results = eng.search("Type", namespace_hint="enterprise_ai_platform.large", max_results=200)
    ...
    def test_large_descendants(self):
        ...
        self._build_large(100)
        eng         = get_query_engine_v2()
        descendants = eng.descendants_of("enterprise_ai_platform.large.Root")
    ...
    def test_large_similar_performance(self):
        ...
        eng = get_query_engine_v2()
        t0  = time.perf_counter()
        eng.similar_types("enterprise_ai_platform.large.Root", top_k=20)
        elapsed = (time.perf_counter() - t0) * 1_000.0
...
class TestEndToEnd:
    def test_full_pipeline(self):
        ...
        types = _build_hierarchy()
        _register_rel(
            "enterprise_ai_platform.test.rel.IssuerOf",
            "IssuerOf",
            "enterprise_ai_platform.test.Equity",
            "enterprise_ai_platform.test.Bond",
        )
        eng = get_query_engine_v2()

        # 1. Basic lookups
        asset = eng.type("enterprise_ai_platform.test.Asset")
        ...
        # 4. Concept expansion
        expanded = eng.expand_concept("enterprise_ai_platform.test.Asset", radius=3)
        ...
        # 6. Relationships
        rels = eng.relationships_for("enterprise_ai_platform.test.Equity")
        assert any(r.name == "IssuerOf" for r in rels)

        # 7. Resolution request
        res = eng.resolution_request("enterprise_ai_platform.test.Bond", ResolutionStrategy.EXACT)
        assert res.succeeded

        # 8. Fluent builder
        result = (
            eng.query(QueryType.DESCENDANTS, "enterprise_ai_platform.test.Asset")
               .limit(10)
               .execute()
        )
        ...
        eng.register_named_query(
            ...
            query_type  = QueryType.TYPE_LOOKUP,
            default_target = "enterprise_ai_platform.test.Equity",
        )
    ...
    def test_round_trip_to_dict(self):
        ...
        # NavigationResult
        nav_res = eng.navigation_request("enterprise_ai_platform.test.Asset", max_depth=2)
        nd      = nav_res.to_dict()
        assert nd["start_uri"] == "enterprise_ai_platform.test.Asset"
        ...
        # SemanticResult
        sem_res = eng.semantic_request("enterprise_ai_platform.test.Asset")
        smd     = sem_res.to_dict()
