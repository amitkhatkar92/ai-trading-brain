...
def _init() :
    from enterprise_ai_platform.ontology import get_ontology_engine, get_ontology_manager
...
class TestConstants:
    ...
    def test_specific_names(self):
        from enterprise_ai_platform.ontology.ontology_constants import (
    ...
    def test_ontology_status_values(self):
        from enterprise_ai_platform.ontology.ontology_constants import OntologyStatus
        assert OntologyStatus.ACTIVE.value == "active"
        assert OntologyStatus.UNLOADED.value == "unloaded"

    def test_type_kind_values(self):
        from enterprise_ai_platform.ontology.ontology_constants import TypeKind
    ...
    def test_cardinality_values(self):
        from enterprise_ai_platform.ontology.ontology_constants import Cardinality
    ...
    def test_string_constants(self):
        from enterprise_ai_platform.ontology.ontology_constants import ONTOLOGY_NAMESPACE, SCHEMA_VERSION
        assert ONTOLOGY_NAMESPACE.startswith("enterprise_ai_platform.")
...
class TestExceptions:
    def test_base_hierarchy(self):
        from enterprise_ai_platform.ontology.ontology_exceptions import (
    ...
    def test_base_has_code_and_message(self):
        from enterprise_ai_platform.ontology.ontology_exceptions import OntologyError
    ...
    def test_not_found_carries_name(self):
        from enterprise_ai_platform.ontology.ontology_exceptions import OntologyNotFoundError
    ...
    def test_already_loaded_carries_name(self):
        from enterprise_ai_platform.ontology.ontology_exceptions import OntologyAlreadyLoadedError
        exc = OntologyAlreadyLoadedError("X")
        assert exc.name == "X"

    def test_type_not_found_carries_uri(self):
        from enterprise_ai_platform.ontology.ontology_exceptions import TypeNotFoundError
        exc = TypeNotFoundError("enterprise_ai_platform.entity.Foo")
        assert exc.uri == "enterprise_ai_platform.entity.Foo"
        assert "ONT-031" in exc.code

    def test_not_initialized_error(self):
        from enterprise_ai_platform.ontology.ontology_exceptions import OntologyNotInitializedError
        exc = OntologyNotInitializedError()
        assert "ONT-051" in exc.code

    def test_circular_inheritance_error_has_chain(self):
        from enterprise_ai_platform.ontology.ontology_exceptions import OntologyCircularInheritanceError
        chain = ["A", "B", "C", "A"]
    ...
    def test_validation_error_has_violations(self):
        from enterprise_ai_platform.ontology.ontology_exceptions import OntologyValidationError
        exc = OntologyValidationError("bad", violations=["v1", "v2"])
        assert exc.violations == ["v1", "v2"]

    def test_runtime_error_is_ontology_error(self):
        from enterprise_ai_platform.ontology.ontology_exceptions import OntologyRuntimeError, OntologyError
...
class TestOntologyContext:
    def test_operation_id_set_in_cm(self):
        from enterprise_ai_platform.ontology.ontology_context import OntologyContext
        ctx = OntologyContext()
        assert ctx.operation_id is None
        with ctx.operation(actor="tester", namespace="enterprise_ai_platform.test"):
            assert ctx.operation_id is not None  # UUID generated
            assert ctx.namespace    == "enterprise_ai_platform.test"
        # Restored after exit
        assert ctx.operation_id is None
    ...
    def test_context_manager_actor(self):
        ...
        from enterprise_ai_platform.ontology.ontology_constants import SYSTEM_ACTOR
...
class TestRuntimeModels:
    def test_namespace_to_dict_roundtrip(self):
        ...
        from enterprise_ai_platform.ontology.ontology_constants import OntologyCategory
        ns = OntologyNamespace(
            uri="enterprise_ai_platform.test", name="Test", prefix="tst",
    ...
    def test_property_to_dict_roundtrip(self):
        from enterprise_ai_platform.ontology.runtime.runtime_object import OntologyProperty
        from enterprise_ai_platform.ontology.ontology_constants import DataType
    ...
    def test_type_def_to_dict_roundtrip(self):
        from enterprise_ai_platform.ontology.runtime.runtime_object import OntologyTypeDef
        from enterprise_ai_platform.ontology.ontology_constants import TypeKind
        td = OntologyTypeDef(
            uri="enterprise_ai_platform.test.MyType",
            name="MyType",
    ...
    def test_relationship_def_to_dict_roundtrip(self):
        ...
        rel = OntologyRelationshipDef(
            uri="enterprise_ai_platform.test.BelongsTo",
            name="BelongsTo",
            namespace_uri="enterprise_ai_platform.test",
            source_type_uri="enterprise_ai_platform.test.A",
            target_type_uri="enterprise_ai_platform.test.B",
        ...
        assert rel2.source_type_uri == "enterprise_ai_platform.test.A"
        assert rel2.target_type_uri == "enterprise_ai_platform.test.B"

    def test_ontology_stats_to_dict(self):
        from enterprise_ai_platform.ontology.runtime.runtime_object import OntologyStats
...
class TestDocumentLoader:
    def test_all_seven_builtins_defined(self):
        from enterprise_ai_platform.ontology.loader.document_loader import _BUILDERS
        from enterprise_ai_platform.ontology.ontology_constants import BUILTIN_ONTOLOGY_NAMES
    ...
    def test_information_ontology_loads(self):
        from enterprise_ai_platform.ontology.loader.document_loader import load_builtin_document
    ...
    def test_entity_ontology_has_types(self):
        from enterprise_ai_platform.ontology.loader.document_loader import load_builtin_document
        from enterprise_ai_platform.ontology.ontology_constants import ONT_ENTITY
        doc = load_builtin_document(ONT_ENTITY)
        assert len(doc.types) > 0

    def test_relationship_ontology_has_relationships(self):
        from enterprise_ai_platform.ontology.loader.document_loader import load_builtin_document
        from enterprise_ai_platform.ontology.ontology_constants import ONT_RELATIONSHIP
...
class TestResourceLoader:
    def test_load_returns_document(self):
        from enterprise_ai_platform.ontology.loader.resource_loader import ResourceLoader
        from enterprise_ai_platform.ontology.loader.document_loader import load_builtin_document
        from enterprise_ai_platform.ontology.ontology_constants import ONT_INFORMATION
...
class TestOntologyLoader:
    ...
    def test_load_builtin_unknown_raises(self):
        from enterprise_ai_platform.ontology.loader.ontology_loader import get_ontology_loader
        from enterprise_ai_platform.ontology.ontology_exceptions import OntologyLoadError
        loader = get_ontology_loader()
    ...
    def test_status_after_load(self):
        from enterprise_ai_platform.ontology.loader.ontology_loader import get_ontology_loader
        from enterprise_ai_platform.ontology.ontology_constants import ONT_KNOWLEDGE, OntologyStatus
        loader = get_ontology_loader()
...
class TestCompiler:
    ...
    def test_circular_inheritance_raises(self):
        from enterprise_ai_platform.ontology.compiler.ontology_compiler import get_ontology_compiler
        from enterprise_ai_platform.ontology.ontology_exceptions import OntologyCircularInheritanceError
        from enterprise_ai_platform.ontology.runtime.runtime_object import (
            OntologyDocument, OntologyNamespace, OntologyTypeDef
        )
        from enterprise_ai_platform.ontology.ontology_constants import (
            OntologyCategory, TypeKind, DEFAULT_ONTOLOGY_VERSION
        )
        ns = OntologyNamespace(uri="enterprise_ai_platform.cycle", name="Cycle", prefix="cyc")
        type_a = OntologyTypeDef(
            uri="enterprise_ai_platform.cycle.A", name="A", namespace_uri="enterprise_ai_platform.cycle",
            kind=TypeKind.CONCRETE, parent_uri="enterprise_ai_platform.cycle.B",
        )
        type_b = OntologyTypeDef(
            uri="enterprise_ai_platform.cycle.B", name="B", namespace_uri="enterprise_ai_platform.cycle",
            kind=TypeKind.CONCRETE, parent_uri="enterprise_ai_platform.cycle.A",
        )
        doc = OntologyDocument(
            uri="enterprise_ai_platform.cycle.ontology", name="CycleTest",
...
class TestRegistryManager:
    ...
    def test_get_type_by_uri(self):
        ...
        td  = mgr.get_type("enterprise_ai_platform.information.BaseObject")
    ...
    def test_get_type_or_none_miss(self):
        from enterprise_ai_platform.ontology.registry.ontology_registry_manager import get_registry_manager
        mgr = get_registry_manager()
        assert mgr.get_type_or_none("enterprise_ai_platform.information.NOPE") is None

    def test_has_type(self):
        from enterprise_ai_platform.ontology.registry.ontology_registry_manager import get_registry_manager
        mgr = get_registry_manager()
        assert mgr.has_type("enterprise_ai_platform.information.NamedObject")

    def test_canonical_uri_by_short_name(self):
        from enterprise_ai_platform.ontology.registry.ontology_registry_manager import get_registry_manager
        mgr = get_registry_manager()
        uri = mgr.canonical_uri("BaseObject")
        assert uri == "enterprise_ai_platform.information.BaseObject"

    def test_ancestors_of_named_object(self):
        from enterprise_ai_platform.ontology.registry.ontology_registry_manager import get_registry_manager
        mgr = get_registry_manager()
        ancestors = mgr.ancestors_of("enterprise_ai_platform.information.NamedObject")
        assert "enterprise_ai_platform.information.BaseObject" in ancestors

    def test_is_subtype_of(self):
        from enterprise_ai_platform.ontology.registry.ontology_registry_manager import get_registry_manager
        mgr = get_registry_manager()
        assert mgr.is_subtype_of("enterprise_ai_platform.information.NamedObject", "enterprise_ai_platform.information.BaseObject")
        assert not mgr.is_subtype_of("enterprise_ai_platform.information.BaseObject", "enterprise_ai_platform.information.NamedObject")

    def test_is_subtype_of_self(self):
        from enterprise_ai_platform.ontology.registry.ontology_registry_manager import get_registry_manager
        mgr = get_registry_manager()
        assert mgr.is_subtype_of("enterprise_ai_platform.information.BaseObject", "enterprise_ai_platform.information.BaseObject")

    def test_children_of(self):
        from enterprise_ai_platform.ontology.registry.ontology_registry_manager import get_registry_manager
        mgr = get_registry_manager()
        children = mgr.children_of("enterprise_ai_platform.information.BaseObject")
        assert "enterprise_ai_platform.information.NamedObject" in children

    def test_descendants_of(self):
        from enterprise_ai_platform.ontology.registry.ontology_registry_manager import get_registry_manager
        mgr = get_registry_manager()
        desc = mgr.descendants_of("enterprise_ai_platform.information.BaseObject")
        assert len(desc) > 0
...
class TestDomainRegistries:
    ...
    def test_entity_registry_has_instrument(self):
        from enterprise_ai_platform.ontology.registry.entity_registry import get_entity_registry
    ...
    def test_event_registry_has_market_event(self):
        from enterprise_ai_platform.ontology.registry.event_registry import get_event_registry
        reg   = get_event_registry()
        names = {t.name for t in reg.all_types()}
        assert "MarketEvent" in names

    def test_observation_registry_has_market_data_observation(self):
        from enterprise_ai_platform.ontology.registry.observation_registry import get_observation_registry
        reg   = get_observation_registry()
        names = {t.name for t in reg.all_types()}
        assert "MarketDataObservation" in names

    def test_knowledge_registry_has_fact(self):
        from enterprise_ai_platform.ontology.registry.knowledge_registry import get_knowledge_ont_registry
        reg   = get_knowledge_ont_registry()
        names = {t.name for t in reg.all_types()}
        assert "Fact" in names

    def test_relationship_registry_has_belongs_to(self):
        from enterprise_ai_platform.ontology.registry.relationship_registry import get_relationship_registry
...
class TestOntologyGraph:
    ...
    def test_shortest_path_parent_child(self):
        from enterprise_ai_platform.ontology.graph.ontology_graph import get_ontology_graph
        graph = get_ontology_graph()
        path = graph.shortest_path(
            "enterprise_ai_platform.information.BaseObject",
            "enterprise_ai_platform.information.NamedObject",
        )
        assert path is not None
        assert path[0]  == "enterprise_ai_platform.information.BaseObject"
        assert path[-1] == "enterprise_ai_platform.information.NamedObject"
    ...
    def test_shortest_path_same_node(self):
        ...
        assert path == ["enterprise_ai_platform.information.BaseObject"]
    ...
    def test_shortest_path_unreachable_is_none(self):
        ...
        path = graph.shortest_path(
            "enterprise_ai_platform.information.BaseObject",
            "enterprise_ai_platform.relationship.BelongsTo",
        )
    ...
    def test_roots(self):
        ...
        assert "enterprise_ai_platform.information.BaseObject" in uris
    ...
    def test_depth_of_base_object(self):
        from enterprise_ai_platform.ontology.graph.ontology_graph import get_ontology_graph
        graph = get_ontology_graph()
        d = graph.depth_of("enterprise_ai_platform.information.BaseObject")
        assert d == 0

    def test_depth_of_child_is_positive(self):
        from enterprise_ai_platform.ontology.graph.ontology_graph import get_ontology_graph
        graph = get_ontology_graph()
        d = graph.depth_of("enterprise_ai_platform.information.NamedObject")
        assert d >= 1
...
class TestOntologyQuery:
    ...
    def test_query_in_namespace(self):
        from enterprise_ai_platform.ontology.query.ontology_query import OntologyQuery
        result = OntologyQuery().in_namespace("enterprise_ai_platform.information").build().execute()
        for td in result:
            assert td.namespace_uri == "enterprise_ai_platform.information"

    def test_query_subtype_of(self):
        from enterprise_ai_platform.ontology.query.ontology_query import OntologyQuery
        result = (
            OntologyQuery()
            .subtype_of("enterprise_ai_platform.information.BaseObject")
            .build()
            .execute()
        )
        # Should find at least NamedObject
        uris = [t.uri for t in result]
        assert "enterprise_ai_platform.information.NamedObject" in uris
...
class TestLookupService:
    ...
    def test_type_by_uri(self):
        from enterprise_ai_platform.ontology.services.lookup_service import get_lookup_service
        svc = get_lookup_service()
        td  = svc.type("enterprise_ai_platform.information.BaseObject")
        assert td.name == "BaseObject"
    ...
    def test_type_by_short_name(self):
        ...
        assert td.uri == "enterprise_ai_platform.information.NamedObject"
    ...
    def test_type_not_found_raises(self):
        ...
        with pytest.raises(TypeNotFoundError):
            svc.type("enterprise_ai_platform.nope.Bogus")

    def test_type_or_none_miss(self):
        from enterprise_ai_platform.ontology.services.lookup_service import get_lookup_service
        svc = get_lookup_service()
        assert svc.type_or_none("enterprise_ai_platform.nope.X") is None

    def test_exists(self):
        from enterprise_ai_platform.ontology.services.lookup_service import get_lookup_service
        svc = get_lookup_service()
        assert svc.exists("enterprise_ai_platform.information.NamedObject")
        assert not svc.exists("enterprise_ai_platform.nope.Y")

    def test_properties_of_includes_inherited(self):
        from enterprise_ai_platform.ontology.services.lookup_service import get_lookup_service
        svc   = get_lookup_service()
        props = svc.properties_of("enterprise_ai_platform.information.NamedObject")
    ...
    def test_is_subtype_of(self):
        from enterprise_ai_platform.ontology.services.lookup_service import get_lookup_service
        svc = get_lookup_service()
        assert svc.is_subtype_of("enterprise_ai_platform.information.NamedObject", "enterprise_ai_platform.information.BaseObject")

    def test_namespace_lookup(self):
        from enterprise_ai_platform.ontology.services.lookup_service import get_lookup_service
        svc = get_lookup_service()
        ns  = svc.namespace("enterprise_ai_platform.information")
        assert ns.uri == "enterprise_ai_platform.information"

    def test_types_in_namespace(self):
        from enterprise_ai_platform.ontology.services.lookup_service import get_lookup_service
        svc   = get_lookup_service()
        types = svc.types_in_namespace("enterprise_ai_platform.information")
        assert len(types) > 0
        for t in types:
            assert t.namespace_uri == "enterprise_ai_platform.information"
...
class TestHierarchyService:
    ...
    def test_build_tree(self):
        from enterprise_ai_platform.ontology.services.hierarchy_service import get_hierarchy_service
        svc  = get_hierarchy_service()
        root = svc.build_tree("enterprise_ai_platform.information.BaseObject")
        assert root.uri  == "enterprise_ai_platform.information.BaseObject"
        assert len(root.children) > 0

    def test_tree_depth(self):
        from enterprise_ai_platform.ontology.services.hierarchy_service import get_hierarchy_service
        svc  = get_hierarchy_service()
        root = svc.build_tree("enterprise_ai_platform.information.BaseObject", max_depth=1)
    ...
    def test_inheritance_chain(self):
        from enterprise_ai_platform.ontology.services.hierarchy_service import get_hierarchy_service
        svc   = get_hierarchy_service()
        chain = svc.inheritance_chain("enterprise_ai_platform.entity.Instrument")
        # Should include the type itself and its ancestors
        uris = [t.uri for t in chain]
        assert "enterprise_ai_platform.entity.Instrument" in uris

    def test_sibling_types(self):
        from enterprise_ai_platform.ontology.services.hierarchy_service import get_hierarchy_service
        svc = get_hierarchy_service()
        # NamedObject's siblings are other direct children of BaseObject
        siblings = svc.sibling_types("enterprise_ai_platform.information.NamedObject")
...
class TestStatisticsService:
    ...
    def test_snapshot_has_fields(self):
        from enterprise_ai_platform.ontology.services.statistics_service import get_statistics_service
...
class TestOntologyFactory:
    def test_create_namespace(self):
        ...
        ns = f.create_namespace("enterprise_ai_platform.myns", "MyNamespace", prefix="my")
        assert ns.uri    == "enterprise_ai_platform.myns"
        assert ns.prefix == "my"
    ...
    def test_create_type(self):
        ...
        from enterprise_ai_platform.ontology.ontology_constants import TypeKind
        f  = get_ontology_factory()
        td = f.create_type("MyType", "enterprise_ai_platform.myns", kind=TypeKind.CONCRETE)
        assert td.name          == "MyType"
        assert td.namespace_uri == "enterprise_ai_platform.myns"
        assert td.kind          == TypeKind.CONCRETE
        assert td.uri           == "enterprise_ai_platform.myns.MyType"
    ...
    def test_create_relationship(self):
        ...
        rel = f.create_relationship(
            "OwnedBy", "enterprise_ai_platform.myns",
            source_type_uri="enterprise_ai_platform.myns.A",
            target_type_uri="enterprise_ai_platform.myns.B",
            cardinality=Cardinality.MANY_TO_ONE,
        )
        assert rel.name            == "OwnedBy"
        assert rel.cardinality     == Cardinality.MANY_TO_ONE
        assert rel.source_type_uri == "enterprise_ai_platform.myns.A"
        assert rel.target_type_uri == "enterprise_ai_platform.myns.B"

    def test_create_document(self):
        from enterprise_ai_platform.ontology.ontology_factory import get_ontology_factory
        from enterprise_ai_platform.ontology.ontology_constants import TypeKind, DataType
        f   = get_ontology_factory()
        ns  = f.create_namespace("enterprise_ai_platform.myns2", "MyNS2")
        td  = f.create_type("Thing", "enterprise_ai_platform.myns2")
...
class TestOntologyRegistry:
    ...
    def test_register_and_status(self):
        from enterprise_ai_platform.ontology.ontology_registry import get_ontology_registry
        from enterprise_ai_platform.ontology.ontology_constants import OntologyStatus
...
class TestOntologyManager:
    def test_initialize_loads_builtins(self):
        from enterprise_ai_platform.ontology.ontology_manager import get_ontology_manager
        from enterprise_ai_platform.ontology.ontology_constants import BUILTIN_ONTOLOGY_NAMES
    ...
    def test_get_type_by_uri(self):
        mgr = _init()
        td  = mgr.get_type("enterprise_ai_platform.entity.Instrument")
        assert td.name == "Instrument"

    def test_get_type_by_short_name(self):
        mgr = _init()
        td  = mgr.get_type("Instrument")
        assert td.uri == "enterprise_ai_platform.entity.Instrument"

    def test_get_type_not_found_raises(self):
        from enterprise_ai_platform.ontology.ontology_exceptions import TypeNotFoundError
        mgr = _init()
        with pytest.raises(TypeNotFoundError):
            mgr.get_type("enterprise_ai_platform.nope.BOGUS")

    def test_exists(self):
        mgr = _init()
        assert mgr.exists("enterprise_ai_platform.entity.Instrument")
        assert not mgr.exists("enterprise_ai_platform.nope.X")

    def test_properties_of(self):
        mgr   = _init()
        props = mgr.properties_of("enterprise_ai_platform.entity.Instrument")
        assert isinstance(props, dict)

    def test_is_subtype_of(self):
        mgr = _init()
        assert mgr.is_subtype_of("enterprise_ai_platform.entity.Instrument", "enterprise_ai_platform.entity.Entity")

    def test_get_namespace(self):
        mgr = _init()
        ns  = mgr.get_namespace("enterprise_ai_platform.entity")
        assert ns.uri == "enterprise_ai_platform.entity"

    def test_types_in_namespace(self):
        mgr   = _init()
        types = mgr.types_in_namespace("enterprise_ai_platform.entity")
        assert len(types) > 0
        assert all(t.namespace_uri == "enterprise_ai_platform.entity" for t in types)

    def test_hierarchy_tree(self):
        mgr  = _init()
        root = mgr.hierarchy_tree("enterprise_ai_platform.entity.Entity")
        assert root.uri == "enterprise_ai_platform.entity.Entity"

    def test_ancestors_of(self):
        mgr  = _init()
        ancs = mgr.ancestors_of("enterprise_ai_platform.entity.Instrument")
        uris = [t.uri for t in ancs]
        assert "enterprise_ai_platform.entity.Entity" in uris

    def test_descendants_of(self):
        mgr   = _init()
        descs = mgr.descendants_of("enterprise_ai_platform.entity.Entity")
    ...
    def test_load_from_dict_custom_ontology(self):
        ...
        ns  = fac.create_namespace("enterprise_ai_platform.custom_test", "CustomTest", prefix="ct",
                                   category=OntologyCategory.EXTENSION)
        td  = fac.create_type("CustomThing", "enterprise_ai_platform.custom_test")
        ...
        assert mgr.exists("enterprise_ai_platform.custom_test.CustomThing")
...
class TestOntologyRuntimeEngine:
    def test_initialize_succeeds(self):
        from enterprise_ai_platform.ontology.ontology_runtime_engine import get_ontology_engine
        engine = get_ontology_engine()
        engine.initialize()
    ...
    def test_manager_property(self):
        from enterprise_ai_platform.ontology.ontology_runtime_engine import get_ontology_engine
        from enterprise_ai_platform.ontology.ontology_manager import OntologyManager
        engine = get_ontology_engine()
        engine.initialize()
    ...
    def test_namespace_attribute(self):
        from enterprise_ai_platform.ontology.ontology_runtime_engine import get_ontology_engine
        from enterprise_ai_platform.ontology.ontology_constants import ONTOLOGY_NAMESPACE
        engine = get_ontology_engine()
...
class TestPublicExports:
    def test_all_symbols_importable(self):
        import enterprise_ai_platform.ontology as ont
        # Core functions
    ...
    def test_model_classes_importable(self):
        from enterprise_ai_platform.ontology import (
    ...
    def test_query_importable(self):
        from enterprise_ai_platform.ontology import OntologyQuery, OntologyQueryResult
...
class TestConcurrency:
    ...
    def test_parallel_queries(self):
        ...
        namespaces = [
            "enterprise_ai_platform.information", "enterprise_ai_platform.entity", "enterprise_ai_platform.event",
            "enterprise_ai_platform.observation", "enterprise_ai_platform.knowledge",
...
class TestEndToEnd:
    def test_full_pipeline_information(self):
        ...
        # Lookup
        td = manager.get_type("enterprise_ai_platform.information.NamedObject")
        assert td.name == "NamedObject"

        # is_subtype_of
        assert manager.is_subtype_of("enterprise_ai_platform.information.NamedObject", "enterprise_ai_platform.information.BaseObject")

        # Hierarchy tree
        root = manager.hierarchy_tree("enterprise_ai_platform.information.BaseObject")
        child_uris = [c.uri for c in root.children]
        assert "enterprise_ai_platform.information.NamedObject" in child_uris

        # Query
        from enterprise_ai_platform.ontology import OntologyQuery
        result = (
            OntologyQuery()
            .in_namespace("enterprise_ai_platform.information")
            .not_abstract()
            .build()
            .execute()
        )
    ...
    def test_full_pipeline_observation(self):
        ...
        from enterprise_ai_platform.ontology import get_ontology_engine, OntologyQuery
        engine = get_ontology_engine()
        engine.initialize()

        result = (
            OntologyQuery()
            .in_namespace("enterprise_ai_platform.observation")
            .subtype_of("enterprise_ai_platform.observation.Observation")
            .build()
            .execute()
        )
        uris = [t.uri for t in result]
        assert "enterprise_ai_platform.observation.MarketDataObservation" in uris
    ...
    def test_full_pipeline_entity(self):
        ...
        assert mgr.is_subtype_of("enterprise_ai_platform.entity.Instrument", "enterprise_ai_platform.entity.Entity")
        instrument = mgr.get_type("enterprise_ai_platform.entity.Instrument")
        assert instrument is not None

        ancestors = mgr.ancestors_of("enterprise_ai_platform.entity.Instrument")
        ancestor_uris = [t.uri for t in ancestors]
        assert "enterprise_ai_platform.entity.Entity" in ancestor_uris

    def test_cross_ontology_relationships(self):
        """Relationship ontology: relationships link types from different ontologies."""
        from enterprise_ai_platform.ontology import get_ontology_engine
        from enterprise_ai_platform.ontology.registry.ontology_registry_manager import get_registry_manager
        engine = get_ontology_engine()
        engine.initialize()
        mgr = get_registry_manager()
