"""
tests/test_company_growth_evidence_log.py
=============================================
Self-learning module #31 (ACQUISITION) -- evidence log for real trade
outcomes tagged with a company_growth_score.

T01  record_company_growth_outcome() persists correctly
...
import learning_system.company_growth_evidence_log as evlog
...
def test_t01_record_persists_correctly():
    evlog.record_company_growth_outcome(
        order_id="O1", symbol="TEST", strategy="breakout",
        company_growth_score=0.5, r_multiple=1.5, won=True,
    )
    records = evlog.get_records()
    assert len(records) == 1
    assert records[0]["company_growth_score"] == 0.5
...
def test_t02_get_records_oldest_first_and_n():
    for i in range(5):
        evlog.record_company_growth_outcome(
            ...
            company_growth_score=0.5, r_multiple=1.0, won=True,
        )
