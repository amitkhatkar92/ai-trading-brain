"""
tests/test_corporate_event_refinement_engine.py
===================================================
Self-learning module #32 (SYNTHESIS + GOVERNANCE) -- evidence-gated
validation of the corporate-event score's predictive value, and its
...
T08  get_corporate_event_adjustment() with no active adjustment == 0.0
T09  get_corporate_event_adjustment(None) == 0.0 (fail-open)
...
import learning_system.corporate_event_refinement_engine as engine
...
def _cohort_records(n_per_cohort, high_win_rate, low_win_rate):
    ...
    for i in range(n_per_cohort):
        recs.append({"timestamp": f"2026-01-{i+1:03d}", "strategy": "X",
                      "corporate_event_score": 0.5, "r_multiple": 1.0 if high_wins[i] else -1.0,
                      "won": high_wins[i]})
        recs.append({"timestamp": f"2026-01-{i+1:03d}b", "strategy": "X",
                      "corporate_event_score": 0.0, "r_multiple": 1.0 if low_wins[i] else -1.0,
                      "won": low_wins[i]})
    return recs
...
def _patch_records(records):
    return patch(
        "learning_system.corporate_event_evidence_log.get_records",
...
def test_t08_default_adjustment_with_no_override():
    assert engine.get_corporate_event_adjustment(0.5) == 0.0


def test_t09_none_score_returns_zero():
    assert engine.get_corporate_event_adjustment(None) == 0.0
...
def test_t10_active_adjustment_only_above_threshold():
    ...
    with _patch_records(_cohort_records(45, 0.90, 0.20)):
        engine.run_daily_refinement_check()
    above = engine.get_corporate_event_adjustment(engine.POSITIVE_THRESHOLD + 0.1)
    assert above > 0.0
    assert above <= engine.MAX_ADJUSTMENT
    below = engine.get_corporate_event_adjustment(engine.POSITIVE_THRESHOLD - 0.1)
    assert below == 0.0
