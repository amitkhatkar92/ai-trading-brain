"""
decision_integration_engine.py — enterprise_ai_platform.decision.integration
============================================================
...
This module:
  ...
  - DOES     publish M5 :class:`~enterprise_ai_platform.decision.snapshot.DecisionSnapshot`
...
from enterprise_ai_platform.investment.workflow.engine_lifecycle import EngineState, LifecycleAwareMixin
...
class DecisionIntegrationEngine(LifecycleAwareMixin):
    ...
    def _execute_workflow(
        ...
        if lc is not None:
            from enterprise_ai_platform.decision.lifecycle.constants import (
                DecisionScope,
                DecisionType,
                DecisionPriority,
                DecisionTrigger,
        ...
        if eng is not None:
            from enterprise_ai_platform.decision.engine import DecisionRequest as EngineRequest
        ...
        if snapshot_store is not None and ctx.session is not None:
            from enterprise_ai_platform.decision.snapshot import DecisionSnapshotBuilder
...
def _extract_candidates(ctx: DecisionIntegrationContext, opt_ctx) -> list:
    ...
    """
    from enterprise_ai_platform.decision.optimization import DecisionCandidate
