"""
decision_transition.py — enterprise_ai_platform.decision.lifecycle
=================================================
Immutable record of a decision session state transition.
...
from .constants import ACTOR_LIFECYCLE, VERSION, DecisionState


@dataclass(frozen=True)
class DecisionTransition:
    """
    Immutable record of a single state transition in a decision session.

    One :class:`DecisionTransition` is appended to the session's
    ...
    session_id :      Decision session that was transitioned.
...
def make_transition(
    ...
    *,
    ...
    """
    Convenience constructor for :class:`DecisionTransition`.
