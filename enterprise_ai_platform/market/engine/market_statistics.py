"""
market_statistics.py — enterprise_ai_platform.market.engine
============================================
Thread-safe atomic counters and derived metrics for the Market Engine.
...
class MarketEngineStatistics:
    """
    Thread-safe statistics accumulator for the Market Engine.
    ...
    - pipelines_failed
    - snapshots_published
    - avg_analysis_time_s   (running mean — Welford)
    - avg_dispatch_time_s   (running mean — Welford)
    - throughput_per_min    (completions in the last 60 s window)
    """
    ...
    def record_pipeline_completed(self, elapsed_s: float = 0.0) -> None:
        with self._lock:
            ...
            self._update_analysis_mean(elapsed_s)
            now = time.time()
    ...
    def _update_analysis_mean(self, elapsed_s: float) -> None:
        self._analysis_count += 1
        delta = elapsed_s - self._analysis_mean
        self._analysis_mean += delta / self._analysis_count
    ...
    def snapshot(self) -> Dict[str, Any]:
        with self._lock:
            ...
            return {
                ...
                "avg_analysis_time_s":  round(self._analysis_mean, 4),
