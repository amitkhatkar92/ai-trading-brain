"""
market_history.py — enterprise_ai_platform.market.lifecycle
===========================================
Bounded history of market lifecycle events and transitions.
...
from .constants import DEFAULT_MAX_HISTORY, DEFAULT_MAX_TRANSITIONS, MarketEventType
...
class MarketHistory:
    ...
    def events_for_exchange(self, exchange: str) -> List:
        """Return all events for a given exchange."""
        with self._lock:
            return [e for e in self._events
                    if getattr(e, "exchange", None) == exchange]

    def events_by_type(self, event_type: MarketEventType) -> List:
    ...
    def record_transition(self, transition: object) -> None:
        """Append a :class:`MarketTransition`."""
        with self._lock:
