"""
market_transition.py — enterprise_ai_platform.market.lifecycle
==============================================
Immutable record of a market session state transition.
...
from .constants import ACTOR_LIFECYCLE, VERSION, MarketState


@dataclass(frozen=True)
class MarketTransition:
    """
    Immutable record of a single state transition in a market session.

    One :class:`MarketTransition` is appended to the session's
    ...
    session_id :      Market session that was transitioned.
...
def make_transition(
    ...
    *,
    ...
    """
    Convenience constructor for :class:`MarketTransition`.
