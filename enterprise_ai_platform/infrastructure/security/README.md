...
```
enterprise_ai_platform/infrastructure/security/
│
├── Core
...
```python
from enterprise_ai_platform.infrastructure.security import get_security_manager
...
```python
from enterprise_ai_platform.infrastructure.security import get_token_manager
...
```python
from enterprise_ai_platform.infrastructure.security import get_session_manager
...
```python
from enterprise_ai_platform.infrastructure.security import get_authorization_manager
...
|---|---|
| `super_admin` | `*` (all) |
| `admin` | `enterprise_ai_platform:admin`, `enterprise_ai_platform:read`, `enterprise_ai_platform:write`, `audit:read` |
| `trader` | `trade:execute`, `trade:read`, `risk:read`, `portfolio:*`, `orders:*` |
| `viewer` | `enterprise_ai_platform:read`, `trade:read`, `risk:read` |
| `risk_manager` | `risk:read`, `risk:override`, `trade:read`, `portfolio:*` |
| `service` | `enterprise_ai_platform:read`, `enterprise_ai_platform:write`, `trade:read`, `orders:*` |

### ABAC (attribute-based policies)

```python
from enterprise_ai_platform.infrastructure.security import get_authorization_manager
from enterprise_ai_platform.infrastructure.security.security_models import PolicyStatement
from enterprise_ai_platform.infrastructure.security.security_constants import PolicyEffect
...
from enterprise_ai_platform.infrastructure.security import get_policy_manager
...
```python
from enterprise_ai_platform.infrastructure.security import get_encryption_manager
...
```python
from enterprise_ai_platform.infrastructure.security import get_key_manager
...
```python
from enterprise_ai_platform.infrastructure.security.security_constants import HashAlgorithm
...
```python
from enterprise_ai_platform.infrastructure.security import get_secret_manager
...
```python
from enterprise_ai_platform.infrastructure.security import InMemoryVaultProvider, EnvironmentVaultProvider
...
# env var IIOS_BROKER_DHAN_API_KEY → path "enterprise_ai_platform/broker/dhan/api_key"
value = sm.get("enterprise_ai_platform/broker/dhan/api_key")
```
...
```python
from enterprise_ai_platform.infrastructure.security import get_integrity_manager, get_tamper_detector
...
```python
from enterprise_ai_platform.infrastructure.security import get_audit_manager, AuditEventType
...
am.key_rotated("enterprise_ai_platform:system", "iios_default")
...
```python
from enterprise_ai_platform.infrastructure.security import get_X_manager, reset_X_manager
...
```python
from enterprise_ai_platform.infrastructure.security import security_scope, system_scope, current_principal_id
...
with system_scope():
    # current_principal_id() → "enterprise_ai_platform:system"
    pass
```
...
```python
from enterprise_ai_platform.infrastructure.security import IdentityProvider
from enterprise_ai_platform.infrastructure.security.principal import Principal
...
# Register:
from enterprise_ai_platform.infrastructure.security import get_identity_manager
...
```python
from enterprise_ai_platform.infrastructure.security import VaultProvider, get_secret_manager
from typing import Optional
