...
```python
from enterprise_ai_platform.investment.strategy.debate import (
...
class SectorRotationAgent(BaseDebateAgent):
    ...
    async def cast_vote(self, context, arguments, registry):
        from enterprise_ai_platform.investment.strategy.debate import make_vote
        return make_vote(
            context.context_id, self.participant_id, self.role,
...
```python
from enterprise_ai_platform.investment.strategy.debate import build_profile, RiskAnalystAgent
