"""
enterprise_ai_platform/observation/enrichment/enrichment_engine.py
=================================================
...
_LOG  = logging.getLogger("enterprise_ai_platform.observation.enrichment.engine")
    ...
    def _enrich(self, obs: Observation, record: EnrichmentRecord, ctx: Any) -> None:
        ...
        if inst:
            _add_link(obs, record, {
                ...
                "ref":   f"enterprise_ai_platform:entity:instrument:{inst.lower()}",
                "label": inst,
            })

        obs_type_link = {
            "type":  LinkType.KNOWLEDGE.value,
            "ref":   f"enterprise_ai_platform:ontology:obs_type:{obs.obs_type.value}",
        ...
        if obs.metadata.domain.value != "unknown":
            _add_link(obs, record, {
                "type":  LinkType.KNOWLEDGE.value,
                "ref":   f"enterprise_ai_platform:ontology:domain:{obs.metadata.domain.value}",
        ...
        if obs.obs_type in (ObservationType.SIGNAL, ObservationType.ORDER_EVENT,
            ...
            if strategy_id:
                _add_link(obs, record, {
                    ...
                    "ref":   f"enterprise_ai_platform:strategy:{strategy_id}",
