"""
enterprise_ai_platform/observation/pipeline/pipeline_engine.py
============================================
...
_LOG    = logging.getLogger("enterprise_ai_platform.observation.pipeline.engine")
...
def _bus():
    try:
        from enterprise_ai_platform.events.event_bus import get_event_bus
...
def _h_publish_events(obs: Observation, ctx: PipelineContext) -> StageResult:
    ...
    try:
        from enterprise_ai_platform.events.event_factory import EventFactory
        ef    = EventFactory()
        event = ef.create(
