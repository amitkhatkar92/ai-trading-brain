"""enterprise_ai_platform/common/async_exec/__init__.py
...
Quick-start::
    ...
    from enterprise_ai_platform.common.async_exec import classify, WorkloadType
    result = classify(my_fn)               # → ClassificationResult

    # 2. Run from sync code (most current IIOS engines):
    from enterprise_ai_platform.common.async_exec import get_execution_manager
    ...
    # 4. Enforce timeouts:
    from enterprise_ai_platform.common.async_exec import apply_timeout, TimeoutPolicy
    policy = TimeoutPolicy(engine_timeout_sec=15.0)
    result = await apply_timeout(coro, policy.engine_timeout_sec)

    # 5. Cancellation:
    from enterprise_ai_platform.common.async_exec import CancellationToken
    token = CancellationToken()
    # from another thread:
    token.cancel("shutdown requested")

    # 6. Migration roadmap:
    from enterprise_ai_platform.common.async_exec import PLATFORM_ASYNC_PROFILES
"""
...
from enterprise_ai_platform.common.async_exec.async_executor import (
    AsyncExecutor,
    ExecutorConfig,
)
