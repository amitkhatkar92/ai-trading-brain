...
Every domain object in `enterprise_ai_platform.workflow.snapshot` implements `.to_dict()` which
...
```python
from enterprise_ai_platform.workflow.snapshot import WorkflowSnapshotEvent, SnapshotEventType
