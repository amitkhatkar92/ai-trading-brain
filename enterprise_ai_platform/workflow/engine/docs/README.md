...
The Workflow Engine (`enterprise_ai_platform.workflow.engine`) is the central coordinator for enterprise workflow execution. It receives workflow requests, validates them, creates M1 lifecycle sessions, schedules and dispatches them through a configurable pipeline, then publishes results.
...
```python
from enterprise_ai_platform.workflow.engine import WorkflowManager, WorkflowEngineFactory
from enterprise_ai_platform.workflow.lifecycle import WorkflowType
from enterprise_ai_platform.workflow.engine import WorkflowDispatchMode
