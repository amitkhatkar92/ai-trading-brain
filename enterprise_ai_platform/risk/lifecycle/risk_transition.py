"""
risk_transition.py — enterprise_ai_platform.risk.lifecycle
==========================================
Immutable record of a risk session state transition.
...
from .constants import ACTOR_LIFECYCLE, VERSION, RiskState


@dataclass(frozen=True)
class RiskTransition:
    """
    Immutable record of a single state transition in a risk session.

    One :class:`RiskTransition` is appended to the session's ``transitions``
    list each time the session moves from one state to another.

    Fields
    ------
    transition_id :   Unique identifier for this transition record.
    session_id :      Risk session that was transitioned.
...
def make_transition(
    ...
    *,
    ...
    """
    Convenience constructor for :class:`RiskTransition`.
