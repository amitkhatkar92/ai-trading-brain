"""
risk_policy_evaluator.py — enterprise_ai_platform.risk.policies
===============================================
Pure structural policy evaluator.

Evaluates conditions and rules against the flat ``inputs`` dict carried by
a :class:`~.risk_policy_request.RiskPolicyRequest`.

No quantitative calculations, no optimization, no broker communication.
...
from .constants import VERSION, ConditionOperator, LogicalOperator
...
class RiskPolicyEvaluator:
    """
    Stateless evaluator for conditions, rules and complete policies.
    ...
    def evaluate_condition(
        self,
        condition: RiskPolicyCondition,
        inputs:    Dict[str, Any],
    ) -> bool:
        """Return True when the condition is satisfied by *inputs*."""
        value = self._resolve_field(condition.field_path, inputs)
        return self._apply_operator(condition.operator, value, condition.threshold, condition.field_path, inputs)
    ...
    def evaluate_policy(
        ...
        """
        from .constants import EvaluationMode, ACTION_SEVERITY
        from .risk_policy_result import RiskPolicyResult

        start = time.perf_counter()
        candidate_results: List[Tuple[int, str, str, List[str], List[str]]] = []
        # (severity, rule_id, rule_name, conditions_met, conditions_failed)
        ...
        # Find the actual action from the rule
        action = policy.default_action
        for rule in policy.rules:
