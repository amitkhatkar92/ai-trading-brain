...
1. Subclass `enterprise_ai_platform.execution.risk.rules.BaseRule`
2. Implement `evaluate(request, context) → RuleResult`
3. Register after manager start:

```python
from enterprise_ai_platform.execution.risk.rules import BaseRule, RuleResult, RuleOutcome
...
```python
from enterprise_ai_platform.execution.risk.controls import PolicyType
