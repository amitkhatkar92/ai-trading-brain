"""enterprise_ai_platform/execution/recovery/lifecycle/recovery_events.py
==================================================
...
def _make_event(
    ...
    session_id: str,
    *,
    actor:    str             = "enterprise_ai_platform:execution:recovery:lifecycle",
    reason:   Optional[str]   = None,
...
def make_recovery_created(session_id: str, *, actor: str = "enterprise_ai_platform:execution:recovery:lifecycle", reason: Optional[str] = None) -> RecoveryEvent:
    return _make_event(RecoveryEventType.RECOVERY_CREATED, session_id, actor=actor, reason=reason)

def make_recovery_initialized(session_id: str, *, actor: str = "enterprise_ai_platform:execution:recovery:lifecycle", reason: Optional[str] = None) -> RecoveryEvent:
    return _make_event(RecoveryEventType.RECOVERY_INITIALIZED, session_id, actor=actor, reason=reason)

def make_recovery_detecting(session_id: str, *, actor: str = "enterprise_ai_platform:execution:recovery:lifecycle", reason: Optional[str] = None) -> RecoveryEvent:
    return _make_event(RecoveryEventType.RECOVERY_DETECTING, session_id, actor=actor, reason=reason)

def make_recovery_assessing(session_id: str, *, actor: str = "enterprise_ai_platform:execution:recovery:lifecycle", reason: Optional[str] = None) -> RecoveryEvent:
    return _make_event(RecoveryEventType.RECOVERY_ASSESSING, session_id, actor=actor, reason=reason)

def make_recovery_ready(session_id: str, *, actor: str = "enterprise_ai_platform:execution:recovery:lifecycle", reason: Optional[str] = None) -> RecoveryEvent:
    return _make_event(RecoveryEventType.RECOVERY_READY, session_id, actor=actor, reason=reason)

def make_recovery_started(session_id: str, *, actor: str = "enterprise_ai_platform:execution:recovery:lifecycle", reason: Optional[str] = None) -> RecoveryEvent:
    return _make_event(RecoveryEventType.RECOVERY_STARTED, session_id, actor=actor, reason=reason)

def make_recovery_verifying(session_id: str, *, actor: str = "enterprise_ai_platform:execution:recovery:lifecycle", reason: Optional[str] = None) -> RecoveryEvent:
    return _make_event(RecoveryEventType.RECOVERY_VERIFYING, session_id, actor=actor, reason=reason)

def make_recovery_completed(session_id: str, *, actor: str = "enterprise_ai_platform:execution:recovery:lifecycle", reason: Optional[str] = None) -> RecoveryEvent:
    return _make_event(RecoveryEventType.RECOVERY_COMPLETED, session_id, actor=actor, reason=reason)

def make_recovery_failed(session_id: str, *, actor: str = "enterprise_ai_platform:execution:recovery:lifecycle", reason: Optional[str] = None) -> RecoveryEvent:
    return _make_event(RecoveryEventType.RECOVERY_FAILED, session_id, actor=actor, reason=reason)

def make_recovery_aborted(session_id: str, *, actor: str = "enterprise_ai_platform:execution:recovery:lifecycle", reason: Optional[str] = None) -> RecoveryEvent:
    return _make_event(RecoveryEventType.RECOVERY_ABORTED, session_id, actor=actor, reason=reason)

def make_recovery_archived(session_id: str, *, actor: str = "enterprise_ai_platform:execution:recovery:lifecycle", reason: Optional[str] = None) -> RecoveryEvent:
