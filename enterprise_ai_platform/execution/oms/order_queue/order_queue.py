"""enterprise_ai_platform/execution/oms/order_queue/order_queue.py
==================================================
...
from enterprise_ai_platform.execution.oms.order_queue.queue_policy import get_policy
...
class OrderQueue(LifecycleAwareMixin):
    ...
    def change_priority(
        ...
        from enterprise_ai_platform.execution.oms.order_queue.constants import QueuePriorityLevel
        self._assert_running()
        entry = self._registry.get(entry_id)
        if entry is None:
            raise QueueEntryNotFoundError(entry_id)
