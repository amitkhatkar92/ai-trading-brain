...
```python
from enterprise_ai_platform.integration.services import IntegrationServicesFactory
...
req = IntegrationServicesFactory.create_rabbitmq_request(
    exchange    = "enterprise_ai_platform.signals",
...
```python
from enterprise_ai_platform.integration.services import ConnectorRequest, ServiceType, TransportType
...
req = ConnectorRequest.create(
    ...
    connector_config    = {
        ...
        "redis_stream_key": "enterprise_ai_platform:trades",
    },
)
response = engine.execute(req)
```
...
```python
from enterprise_ai_platform.integration.services import QueueEngine, MessageDeliveryMode
...
```python
from enterprise_ai_platform.integration.services import EventBusEngine
