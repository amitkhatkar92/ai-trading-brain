"""
redis_stream_adapter.py — enterprise_ai_platform.integration.services
-----------------------------------------------------
...
class BaseRedisStreamAdapter(ABC):
    ...
    def execute(self, request: ConnectorRequest) -> ConnectorResponse:
        ...
        try:
            ...
            key       = cfg.get("redis_stream_key", "enterprise_ai_platform:stream")
            if operation == "xread":
                entries = self.xread(
                    stream_key=key,
                    consumer_group=cfg.get("redis_consumer_group", "enterprise_ai_platform-group"),
                    consumer=cfg.get("redis_consumer", "enterprise_ai_platform-consumer"),
