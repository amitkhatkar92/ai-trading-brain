"""
integration_history.py — enterprise_ai_platform.integration.engine
--------------------------------------------------
Bounded, append-only history of integration requests and responses.
...
class IntegrationEngineHistory:
    """
    Thread-safe, bounded, append-only history of integration activity.
    ...
    def __init__(self, max_history: int = DEFAULT_MAX_HISTORY) -> None:
        ...
        self._requests:    Deque[IntegrationRequest]      = deque(maxlen=max_history)
        self._responses:   Deque[IntegrationResponse]     = deque(maxlen=max_history)
        self._req_idx:     Dict[str, IntegrationRequest]  = {}
        self._resp_idx:    Dict[str, IntegrationResponse] = {}
    ...
    def record_request(self, request: IntegrationRequest) -> None:
        with self._lock:
    ...
    def record_response(self, response: IntegrationResponse) -> None:
        with self._lock:
    ...
    def get_request(self, request_id: str) -> Optional[IntegrationRequest]:
        with self._lock:
            return self._req_idx.get(request_id)

    def get_response(self, response_id: str) -> Optional[IntegrationResponse]:
        with self._lock:
            return self._resp_idx.get(response_id)

    def recent_requests(self, n: int = 20) -> List[IntegrationRequest]:
        with self._lock:
            return list(self._requests)[-n:]

    def recent_responses(self, n: int = 20) -> List[IntegrationResponse]:
        with self._lock:
            return list(self._responses)[-n:]

    def by_session(self, session_id: str) -> List[IntegrationResponse]:
        with self._lock:
