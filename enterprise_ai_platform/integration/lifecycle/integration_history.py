"""
integration_history.py — enterprise_ai_platform.integration.lifecycle
-----------------------------------------------------
Append-only, bounded history of lifecycle transitions and state records.
...
class IntegrationHistory:
    """
    Thread-safe, bounded, append-only history of integration lifecycle events.

    Maintains two separate bounded buffers:
      _transitions   — IntegrationTransition records
      _state_records — IntegrationStateRecord records
    ...
    def __init__(
        ...
        self._transitions:   Deque[IntegrationTransition]  = deque(maxlen=max_transitions)
        self._state_records: Deque[IntegrationStateRecord] = deque(maxlen=max_history)

        self._by_session_transitions:   Dict[str, List[str]] = {}
        self._by_session_state_records: Dict[str, List[str]] = {}

        self._transition_idx:   Dict[str, IntegrationTransition]  = {}
        self._state_record_idx: Dict[str, IntegrationStateRecord] = {}
    ...
    def record_transition(self, transition: IntegrationTransition) -> None:
        with self._lock:
    ...
    def record_state(self, state_record: IntegrationStateRecord) -> None:
        with self._lock:
    ...
    def recent_transitions(self, n: int = 20) -> List[IntegrationTransition]:
        with self._lock:
            return list(self._transitions)[-n:]

    def recent_state_records(
        self, n: int = 20
    ) -> List[IntegrationStateRecord]:
        with self._lock:
    ...
    def session_count(self) -> int:
        with self._lock:
            return len(self._by_session_transitions)
