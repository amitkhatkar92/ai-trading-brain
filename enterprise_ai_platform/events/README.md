...
**Package:** `enterprise_ai_platform/events/`  
...
```python
from enterprise_ai_platform.events import get_event_manager, reset_event_manager
...
```python
from enterprise_ai_platform.events import SagaWorkflow
...
Layer 17 — ControlTower
    │
    ▼
enterprise_ai_platform/events/          ← THIS PACKAGE
    │   EventBus, EventManager
    │   CommandBus, QueryBus, ResponseBus
    │   WorkflowEngine (Pipeline + Saga)
    │
    ▼
enterprise_ai_platform/infrastructure/events/   ← SEPARATE (basic bus, do not modify)
```
