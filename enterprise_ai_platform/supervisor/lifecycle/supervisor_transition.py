"""
supervisor_transition.py — enterprise_ai_platform.supervisor.lifecycle
=====================================================
Immutable record of a supervisor session state transition.
...
from .constants import ACTOR_LIFECYCLE, VERSION, SupervisorState


@dataclass(frozen=True)
class SupervisorTransition:
    """
    Immutable record of a single state transition in a supervisor session.

    One :class:`SupervisorTransition` is appended to the session's
    ``transitions`` list each time the session moves from one state to
    another.

    Fields
    ------
    transition_id :   Unique identifier for this transition record.
    session_id :      Supervisor session that was transitioned.
...
def make_transition(
    ...
    *,
    ...
    """
    Convenience constructor for :class:`SupervisorTransition`.
