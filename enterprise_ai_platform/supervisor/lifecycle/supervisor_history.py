"""
supervisor_history.py — enterprise_ai_platform.supervisor.lifecycle
--------------------------------------------------
Bounded history of supervisor lifecycle events and transitions.
...
from .constants import DEFAULT_MAX_HISTORY, DEFAULT_MAX_TRANSITIONS, SupervisorEventType
...
class SupervisorHistory:
    ...
    def record_event(self, event: object) -> None:
        """Append a :class:`SupervisorEvent`."""
        with self._lock:
    ...
    def events_for_supervisor(self, supervisor_id: str) -> List:
        """Return all events for a given supervisor_id."""
        with self._lock:
            return [e for e in self._events
                    if getattr(e, "supervisor_id", None) == supervisor_id]

    def events_by_type(self, event_type: SupervisorEventType) -> List:
    ...
    def record_transition(self, transition: object) -> None:
        """Append a :class:`SupervisorTransition`."""
        with self._lock:
    ...
    def clear(self) -> None:
        """Discard all retained events and transitions."""
        with self._lock:
            self._events.clear()
