"""
learning_system/corporate_event_refinement_engine.py
========================================================
Self-learning module #32 (SYNTHESIS + GOVERNANCE) -- Event
Intelligence auto-validation and KDA wiring.

Validates, purely from real closed-trade evidence recorded by
corporate_event_evidence_log.py, whether a strongly-positive corporate-
event score (real NSE announcements, keyword-classified, see
opportunity_engine/corporate_event_signal.py) actually correlates with
...
company_growth_refinement_engine.py.
...
WIRING INTO KDA
----------------
get_corporate_event_adjustment(score) is consumed by
knowledge_authority/knowledge_decision_authority.py's
_compute_authority() as a FOURTH, independent, bounded +/-0.05 nudge
to `relevance` (alongside the ARS bridge, institutional-flow bridge,
and company-growth bridge). Zero effect (0.0) until validated --
dormant on deploy.

SAFETY CONTRACT
-----------------
Zero imports of execution_engine, order_manager, broker APIs.
get_corporate_event_adjustment() is a pure, read-only accessor; falls
...
ACTOR = "CORPORATE-EVENT-RE-001"

_ROOT       = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_STORE_DIR  = os.path.join(_ROOT, "data", "learning_system", "corporate_event_calibration")
...
def _append_ledger(event_type: str, reason: str, **extra: Any) -> None:
    try:
        ...
        log.debug("[CorporateEventRE] ledger write failed: %s", exc)
...
def _cohort_split(recs: List[Dict[str, Any]]) -> Tuple[List[bool], List[bool]]:
    high = [bool(r["won"]) for r in recs if r["corporate_event_score"] > POSITIVE_THRESHOLD]
    low  = [bool(r["won"]) for r in recs if r["corporate_event_score"] <= POSITIVE_THRESHOLD]
    return high, low
...
def get_corporate_event_adjustment(score: Optional[float]) -> float:
    """
    Return the bounded relevance nudge (+/- MAX_ADJUSTMENT) for a given
    corporate_event_score. Returns 0.0 whenever: score is None, no
    ...
    try:
        ...
        log.debug("[CorporateEventRE] get_corporate_event_adjustment fallback to 0.0: %s", exc)
        return 0.0
...
def run_daily_refinement_check() -> Dict[str, Any]:
    try:
        ...
        log.debug("[CorporateEventRE] refinement check error: %s", exc)
        return {"status": "ERROR", "error": str(exc)}


def _run_impl() -> Dict[str, Any]:
    from learning_system.corporate_event_evidence_log import get_records
