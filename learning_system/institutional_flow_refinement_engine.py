"""
learning_system/institutional_flow_refinement_engine.py
===========================================================
Self-learning module #30 (SYNTHESIS + GOVERNANCE) -- Positioning
Intelligence auto-validation and KDA wiring.

Validates, purely from real closed-trade evidence recorded by
institutional_flow_evidence_log.py, whether a strongly-positive
institutional-flow score (net bulk/block-deal accumulation + rising
delivery %, see opportunity_engine/institutional_flow_signal.py)
actually correlates with a higher win rate than trades with a low/
negative score. Splits into a "high score" cohort (score > POSITIVE_
THRESHOLD) vs a "low/negative score" cohort and validates the win-rate
DIFFERENCE via the same time-ordered 70/30 + bootstrap-CI scorecard
already used by learning_system/sizing_bounds_refinement_engine.py.

LIFECYCLE (fully automated, no human step at any transition -- mirrors
sizing_bounds_refinement_engine.py exactly):
...
WIRING INTO KDA
----------------
get_institutional_flow_adjustment(score) is consumed by
knowledge_authority/knowledge_decision_authority.py's
_compute_authority(), mirroring the existing ARS->KDA bridge
(HypothesisRegistry.get_confirmed_adjustment) exactly: a second,
independent, bounded +/-0.05 nudge to `relevance`, applied only when
this engine's own real evidence has validated it. Zero effect (0.0)
until validated -- dormant on deploy, since no evidence exists yet.

SAFETY CONTRACT
-----------------
Zero imports of execution_engine, order_manager, broker APIs.
get_institutional_flow_adjustment() is a pure, read-only accessor;
falls back to 0.0 (no adjustment) on any error, missing file, or no
active adjustment. Never imports or modifies equity_scanner_ai.py or
...
ACTOR = "INSTITUTIONAL-FLOW-RE-001"

_ROOT       = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_STORE_DIR  = os.path.join(_ROOT, "data", "learning_system", "institutional_flow_calibration")
...
COOLDOWN_DAYS             = 30
MAX_ADJUSTMENT            = 0.05   # bounded, matches the existing ARS->KDA bridge's own cap
...
def _append_ledger(event_type: str, reason: str, **extra: Any) -> None:
    try:
        ...
        log.debug("[InstitutionalFlowRE] ledger write failed: %s", exc)
...
# ─────────────────────────────────────────────────────────────────────────────
# Statistical validation -- cohort win-rate diff (mirrors sizing_bounds_
# refinement_engine.py's shape exactly)
# ─────────────────────────────────────────────────────────────────────────────
...
def _cohort_split(recs: List[Dict[str, Any]]) -> Tuple[List[bool], List[bool]]:
    high = [bool(r["won"]) for r in recs if r["institutional_flow_score"] > POSITIVE_THRESHOLD]
    low  = [bool(r["won"]) for r in recs if r["institutional_flow_score"] <= POSITIVE_THRESHOLD]
    return high, low
...
def get_institutional_flow_adjustment(score: Optional[float]) -> float:
    """
    Return the bounded relevance nudge (+/- MAX_ADJUSTMENT) for a given
    institutional_flow_score. Returns 0.0 whenever: score is None, no
    active validated adjustment exists, or the score does not clear the
    ...
    try:
        ...
        log.debug("[InstitutionalFlowRE] get_institutional_flow_adjustment fallback to 0.0: %s", exc)
        return 0.0
...
def run_daily_refinement_check() -> Dict[str, Any]:
    try:
        ...
        log.debug("[InstitutionalFlowRE] refinement check error: %s", exc)
        return {"status": "ERROR", "error": str(exc)}


def _run_impl() -> Dict[str, Any]:
    from learning_system.institutional_flow_evidence_log import get_records
