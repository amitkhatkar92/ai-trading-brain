...
class BloombergProvider(BaseNewsProvider):
    """Bloomberg news provider — scaffold only. All data methods raise NotImplementedError."""
    ...
    async def fetch_articles(self, **kwargs):
        raise NotImplementedError("BloombergProvider.fetch_articles not yet wired.")

    async def fetch_events(self, **kwargs):
        raise NotImplementedError("BloombergProvider.fetch_events not yet wired.")

    async def search_news(self, **kwargs):
        raise NotImplementedError("BloombergProvider.search_news not yet wired.")

    async def stream_news(self) -> AsyncGenerator[NewsArticle, None]:
        raise NotImplementedError("BloombergProvider.stream_news not yet wired.")
        if False:
            yield NewsArticle()

    async def stream_alerts(self) -> AsyncGenerator[NewsHeadline, None]:
        raise NotImplementedError("BloombergProvider.stream_alerts not yet wired.")
        if False:
            yield NewsHeadline()
