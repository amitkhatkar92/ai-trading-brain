...
class MasterOrchestrator:
    ...
    def __init__(self):
        ...
        # Cross-wire: OrderManager needs TradeMonitor to deregister replaced positions;
        # TradeMonitor already holds OrderManager ref via inject_order_manager (called below).
        ...
        # Inject OrderManager into CycleHealthMonitor so it can distinguish
        # legitimate in-memory carries from true CSV orphans when computing
        # STALE_POSITIONS. Must happen after both are initialised.
        try:
            self.control_tower.health.inject_order_manager(self.order_manager)
        except Exception as _chm_exc:
            log.warning("[Orchestrator] CycleHealthMonitor OM injection failed: %s", _chm_exc)
    ...
    def _market_close_notify(self) -> None:
            ...
            if open_orders:
                ...
                for rec in open_orders:
                    try:
                        ...
                        q = _gfm().get_quote(f"{rec.symbol}.NS")
        ...
        try:
            ...
            try:
                ...
                pass
            _nifty = self._get_nifty_str()
            # Restore integrity snapshot (carries surviving restarts)
            _rs = ""
            try:
                rs = self.order_manager.get_restore_stats()
                if rs.get("restored_carry", 0) or rs.get("orphan_watch", 0):
                    _rs = (
                        f"\nRestore integrity: "
                        f"today={rs['restored_today']} carry={rs['restored_carry']} "
                        f"closed={rs['skipped_closed']} orphan_watch={rs['orphan_watch']}"
                    )
            except Exception:
                pass
            n.market_alert(
    ...
    def start_scheduler(self) -> None:
        ...
        try:
            ...
            try:
                rs = self.order_manager.get_restore_stats()
                total_r = rs.get("restored_today", 0) + rs.get("restored_carry", 0)
                if total_r or rs.get("orphan_watch", 0):
                    _rs_ping = (
                        f"\nRestore: today={rs['restored_today']} carry={rs['restored_carry']} "
                        f"orphan_watch={rs['orphan_watch']} closed={rs['skipped_closed']}"
                    )
            except Exception:
                pass
