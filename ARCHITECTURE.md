...
| **IIOS** (historical name, resolved) | Renamed `iios/` → `enterprise_ai_platform/` on 2026-09-14 (Plan B Phase 4) specifically to remove the OIOS/IIOS collision | ❌ No — zero imports anywhere in the trading pipeline | A separate, still pre-"Wave 1" generic Enterprise AI Platform framework (see `A1`–`A10_*_IMPLEMENTATION_REPORT.md`). Package name, all internal imports (~6,000 files: the package itself + its mirrored `tests/ai/`, `tests/unit/*`, `tests/certification/`, `tests/performance/` suites + `pyproject.toml` + `.pre-commit-config.yaml` + `bootstrap.py`/`dev.py`/`run.py`/`healthcheck.py`) and CLI entry points were mechanically renamed. Verified via `compileall` (clean) and `pytest --collect-only` (34,554 tests collected, 0 errors) before/after. Uppercase "IIOS" prose mentions in historical root `.md` reports (A1–A10, `AI_PLATFORM_*.md`, root `README.md`, etc) were deliberately left untouched per the Phase 3 "no retroactive doc edits" precedent — this row now describes a resolved, historical collision, not a live one. |
...
| **`knowledge_system/`** (repo root package) | `knowledge_system/` | ✅ Yes — 6 import sites in `master_orchestrator.py` | OPTIONS-ONLY knowledge/pattern/hypothesis subsystem (`options_knowledge_store.py`, `options_pattern_engine.py`, `options_research_pipeline.py`, etc). **Not the same package as KSL's `scripts/knowledge_system/` above** — identical folder name, two unrelated live packages, distinguishable only by full path. Always write the full path when referring to either. |
| **PRR** | `production_readiness/` | ❌ No — 9 certification gates (`ph1_edge_gate.py`…`ph9_certification.py`) computed but never enforced anywhere | Production Readiness Reporter — advisory certification scorecard only; `certification_status` never gates trading. |
| **PGA** | `predictive_gap/` | ❌ No | Predictive Gap Analysis — imports ARS's `KnowledgeProvider`; downstream of an already-disconnected subsystem. |
| **CLE** | `cle_learning_executor/` | Research-only | Learning executor for outlier ("Cat-E") moves the platform/ARS had zero DNA coverage for. |
| **GVA** | `growth_validator/` | ❌ No | Growth Validator — "has the system become more intelligent" metrics; standalone, own reporter. |
| **HKAP** | `hkap/` | ❌ No | Own docstring claims it "reuses the complete IIOS V1.0 platform" — that premise was never true; `enterprise_ai_platform/` (formerly `iios/`) is only a Wave-1 placeholder (see IIOS row above), so HKAP's foundation was aspirational, not real. |
| **IKN** | `ikn/` | ❌ No | Standalone knowledge-network store/query (`ikn_network.py`, `ikn_store.py`, `ikn_query.py`) — no consumer outside its own reports. |
| **"DTA" (two unrelated meanings)** | (a) `decision_tracer/`'s internal modules, renamed `dta_*.py` → `dtrace_*.py` on 2026-09-14 (Plan B Phase 2) specifically to remove this collision; (b) this repo's ticket/incident naming convention used in hundreds of commits and reports (`DTA-040-C`, `DTA-KDA-CONV-002`, `DTA-COALINDIA-RECONCILE-001`, etc — see the Files Modified log in `.github/copilot-instructions.md`) | `decision_tracer/`: ❌ No | Same 3-letter token, two unrelated meanings. `decision_tracer/` was fully disconnected (0 live imports, 4 files), which is why it was the first and — so far — only rename executed from this glossary. Older reports predating 2026-09-14 (e.g. `MARKET_OPPORTUNITY_COVERAGE_AUDIT.md`) still say `decision_tracer.dta_runner` — historical snapshots, not updated retroactively; the current module is `decision_tracer.dtrace_runner`. Never assume a bare "DTA" reference means the ticket convention — check whether the context is `decision_tracer/` first. |
...
**Standing rule, going forward (added 2026-09-14, Plan B Phase 3):** do not use "IIOS" as an informal nickname for this trading system (`ai_trading_brain`) in any NEW doc, docstring, script, or report. Refer to the project by its real name, or name the specific subsystem you mean (`oios/`, `knowledge_authority/`, etc). Existing pre-2026-09-14 docs that already use "IIOS" this way are left as historical artifacts, not retroactively edited.

**Phase 4 completed (2026-09-14):** the `iios/` package itself was renamed to `enterprise_ai_platform/`, which permanently resolves the OIOS/IIOS package-path collision at the source rather than just documenting around it. Any remaining lowercase `iios` you see anywhere in the repo is either (a) inside a pre-2026-09-14 historical `.md` report (intentionally untouched), or (b) uppercase "IIOS" prose/env-var-prefix usage (`IIOS_ENV`, `IIOS-FCR-001`, `IIOSError`), which was deliberately out of scope for this phase (see rule above).
