...
| **CLE** | `cle_learning_executor/` | Research-only | Learning executor for outlier ("Cat-E") moves IIOS/ARS had zero DNA coverage for. |
| **GVA** | `growth_validator/` | ❌ No | Growth Validator — "has the system become more intelligent" metrics; standalone, own reporter. |
| **HKAP** | `hkap/` | ❌ No | Own docstring claims it "reuses the complete IIOS V1.0 platform" — that premise was never true; `iios/` is only a Wave-1 placeholder (see IIOS row above), so HKAP's foundation was aspirational, not real. |
