...
| **"DTA" (two unrelated meanings)** | (a) `decision_tracer/`'s internal file prefix (`dta_analyzer.py`, `dta_collector.py`, `dta_reporter.py`, `dta_runner.py`); (b) this repo's ticket/incident naming convention used in hundreds of commits and reports (`DTA-040-C`, `DTA-KDA-CONV-002`, `DTA-COALINDIA-RECONCILE-001`, etc — see the Files Modified log in `.github/copilot-instructions.md`) | `decision_tracer/`: ❌ No | Same 3-letter token, two unrelated meanings — highest grep-confusion risk found in this audit. `decision_tracer/` is fully disconnected (0 live imports, 4 files), making it the single lowest-risk rename candidate in this whole glossary if/when a rename is ever explicitly authorized. Never assume a bare "DTA" reference means the ticket convention — check whether the context is `decision_tracer/` first. |
